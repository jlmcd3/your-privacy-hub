...
const PREVIEW_EXCERPTS: Record<string, Record<string, { headline: string; excerpt: string; action: string }>> = {
  ...
  adtech: {
    eu: {
      ...
      excerpt:
        "The Belgian DPA's follow-up ruling on the IAB's Transparency & Consent Framework, issued alongside CNIL's enforcement against two programmatic platforms, signals a coordinated European push against TCF-reliant consent mechanisms. If your ad stack relies on TCF 2.2 for EU consent signals, this week's actions suggest your legal basis is under active review by at least three DPAs.",
      action:
        "Legal basis audit required for all EU programmatic inventory by end of Q2. Document your legitimate interest assessments for AdTech use cases now.",
    },
    us: {
      headline: "FTC finalizes commercial surveillance rules — AdTech implications are immediate",
      excerpt:
        "This week's FTC final rule on commercial surveillance contains specific provisions targeting behavioral advertising data brokers and lookalike audience construction. The rule's definition of 'sensitive data' now explicitly includes inferred demographic data — a category most AdTech platforms generate through audience modeling — triggering opt-in consent requirements before July enforcement.",
      action:
        "Map all inferred demographic data flows against the FTC's new sensitive data definition. Audiences built from modeled data likely require explicit consent under the new rule.",
    },
    global: {
      headline: "Cookie consent divergence widens: EU, UK, and US now have three incompatible standards",
      excerpt:
        "CNIL's new cookie guidance, the ICO's revised cookie compliance checker, and California's CPPA technical specifications published within 10 days of each other — and they are not reconcilable without jurisdiction-specific consent flows. Unified consent management is no longer viable for multinational publishers without technical segmentation by user geography.",
      action:
        "Consent management platforms must implement geography-aware consent flows by July 1. A single consent banner cannot comply with all three regimes simultaneously.",
    },
    apac: {
      headline: "Japan and South Korea align on cross-border ad data restrictions",
      excerpt:
        "Japan's PPC and South Korea's PIPC issued coordinated guidance on cross-border advertising data transfers this week, creating new consent requirements for programmatic platforms operating across APAC markets. The alignment creates a de facto APAC standard that diverges from both EU and US approaches.",
      action:
        "Review APAC programmatic ad data flows for compliance with Japan-Korea aligned standards. Separate consent mechanisms may be required.",
    },
  },
  fintech: {
    eu: {
      ...
      excerpt:
        "This week's EBA technical standard under DORA requires financial institutions to maintain detailed AI model documentation that includes training data provenance — creating a direct tension with GDPR data minimization principles. Firms that deleted training data to comply with GDPR retention requirements may now be non-compliant with DORA's model explainability obligations.",
      action:
        "Map your AI model documentation against both DORA Article 28 requirements and your GDPR deletion schedules. Legal and technology teams must co-own this review.",
    },
    us: {
      headline: "CFPB Section 1033 open banking rule creates new privacy consent architecture",
      excerpt:
        "The CFPB's final rule on consumer financial data access creates a new consent framework that operates alongside but not within GLBA — meaning financial institutions must now maintain two parallel consent systems for the same data. The rule's 'authorized third party' definition overlaps substantially with CCPA's service provider definition but uses different eligibility criteria.",
      action:
        "Your privacy notice and consent flows need dual-track redesign: one for GLBA covered data, one for Section 1033 data portability. Deadline is January 2026 for covered institutions.",
    },
    global: {
      headline: "Cross-border financial data: BIS, FSB, and EU diverge on AI risk and privacy",
      excerpt:
        "Three simultaneous developments from the Bank for International Settlements, the Financial Stability Board, and the EU AI Act implementation team this week produced conflicting guidance on AI explainability for credit decisions. Multinational financial institutions now face genuinely incompatible regulatory expectations across their AI governance frameworks.",
      action:
        "Your AI governance policy needs jurisdiction-specific annexes. A single global AI policy cannot satisfy BIS, FSB, and EU AI Act requirements simultaneously.",
    },
    apac: {
      headline: "Hong Kong and Singapore fintech regulators tighten AI model governance requirements",
      excerpt:
        "HKMA and MAS simultaneously released updated AI governance frameworks for financial institutions, requiring explainability documentation and bias testing for credit scoring models. The dual regulatory action creates new compliance obligations for APAC fintech firms operating across both jurisdictions.",
      action:
        "Review AI model governance documentation against both HKMA and MAS updated frameworks. Cross-jurisdiction compliance mapping is critical for Q3.",
    },
  },
  ai: {
    eu: {
      ...
      excerpt:
        "The EU AI Act's prohibited practices provisions became enforceable this week, and General Purpose AI model providers face their first formal compliance deadline for systemic risk assessments. Three major LLM providers received formal requests from the AI Office for model capability documentation under Article 55 — the first use of the AI Office's investigatory powers.",
      action:
        "GPAI providers must file systemic risk assessments with the EU AI Office within 30 days of this week's formal requests. Downstream deployers must review provider compliance status.",
    },
    us: {
      headline: "FTC AI guidance targets 'dark patterns' in AI disclosure as NIST AI RMF gains regulatory force",
      excerpt:
        "The FTC's new policy statement on deceptive AI practices, combined with the NIST AI Risk Management Framework being incorporated by reference into three state AI laws this week, creates a de facto national standard for AI disclosure that is now enforceable. The FTC's 'material AI use' definition is broader than most industry disclosures currently meet.",
      action:
        "Audit all customer-facing AI disclosures against the FTC's new 'material AI use' standard. The threshold is broader than most legal teams assumed — nearly all chatbot and recommendation system uses must be disclosed.",
    },
    global: {
      headline: "G7 AI Code of Conduct gains teeth as Japan, UK, and Canada move to implement simultaneously",
      excerpt:
        "This week's coordinated announcement by Japan, UK, and Canada to incorporate the G7 Hiroshima AI Code of Conduct into domestic regulation creates the first substantive multinational AI governance framework with enforcement implications. Organizations with significant operations in all three jurisdictions now face a 12-month implementation runway before the soft law becomes hard obligations.",
      action:
        "Map your AI systems against the G7 Code's 11 principles. Japan's implementation timeline is most aggressive — domestic guidance expected within 90 days.",
    },
    apac: {
      headline: "China's AI safety regulations enter enforcement phase with first compliance reviews",
      excerpt:
        "China's Cyberspace Administration began conducting the first formal compliance reviews under the Generative AI Measures this week, targeting both domestic and foreign-operated AI services accessible to Chinese users. The reviews focus on content safety, data training transparency, and algorithmic recommendation accountability.",
      action:
        "If your AI services are accessible to users in China, prepare for CAC compliance review. Documentation of training data sources and content safety measures is now mandatory.",
    },
  },
  retail: {
    eu: {
      ...
      excerpt:
        "The European Commission confirmed this week that major retail marketplace operators meeting the 'very large online platform' threshold under the DSA must now comply with enhanced transparency and algorithmic accountability requirements — including data access for researchers and detailed explanation obligations for recommendation systems. Most retail operators had been treating DSA as a social media regulation.",
      action:
        "If your marketplace has >45M EU monthly users, DSA compliance is not optional and enforcement is active. The Commission issued its first formal requests to non-social-media platforms this week.",
    },
    us: {
      headline: "Children's privacy trifecta: COPPA 2.0, state kids' codes, and FTC enforcement converge",
      excerpt:
        "Three simultaneous developments create maximum children's privacy exposure for retail platforms: COPPA 2.0's Senate passage, California's Age-Appropriate Design Code enforcement escalation, and the FTC's specific warning to e-commerce operators about targeted advertising to under-18 users. Any retail platform with children as a foreseeable user segment faces all three simultaneously.",
      action:
        "Children's data audit required immediately. The FTC's definition of 'directed to children' in retail contexts is broader than most legal teams assume — covers platforms where children are a reasonably foreseeable user, not just a primary audience.",
    },
    global: {
      headline: "Cross-border e-commerce: EU, UK, US, and China have fundamentally different adequacy frameworks",
      excerpt:
        "This week's developments in EU-US data flows (post-Schrems II adequacy review), UK's independent adequacy framework divergence, and China's new cross-border data transfer certification requirements collectively mean that a multinational retailer with customer data across all four regions cannot maintain a single data transfer mechanism. Each requires jurisdiction-specific legal instruments.",
      action:
        "Data transfer mapping is now mandatory for any retailer with customers in all four regions. Standard Contractual Clauses alone are no longer sufficient for China outbound transfers.",
    },
    apac: {
      ...
      action:
        "Map your Indian customer data flows against DPDP Act requirements. Payment data localization and marketing consent requirements are the highest priority items.",
    },
  },
  legal: {
    eu: {
...
export default function ProBriefPreview() {
  ...
  const preview = industry && region ? (PREVIEW_EXCERPTS[industry]?.[region] ?? null) : null;
      ...
      {!preview && (
        <div className="bg-fog rounded-xl p-6 text-center">
          <p className="text-slate text-[13px]">
            Select your sector and region above to see what your tailored Monday brief would have opened with this week.
          </p>
        </div>
      )}
        ...
        <div className="bg-gradient-to-br from-navy to-steel rounded-xl p-6 text-white">
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            <span className="text-[9px] font-bold uppercase tracking-widest text-amber-400 bg-amber-400/10 border border-amber-400/20 px-2.5 py-0.5 rounded-full">
              ⭐ Your Intelligence Brief — This Week
            </span>
            <span className="text-blue-300/60 text-[10px]">Sample based on this week's actual developments</span>
          </div>

          {/* Headline */}
          <h3 className="font-display font-bold text-white text-[16px] leading-snug mb-3">{preview.headline}</h3>

          {/* Excerpt */}
          <p className="text-blue-100/80 text-[13px] leading-relaxed mb-4">{preview.excerpt}</p>
          ...
          <div className="bg-white/10 border border-white/15 rounded-lg px-4 py-3">
            ...
            <p className="text-white/90 text-[13px] leading-relaxed">{preview.action}</p>
          </div>

          {/* Conversion CTA */}
          <div className="mt-5 pt-4 border-t border-white/10 flex items-center justify-between flex-wrap gap-3">
            <p className="text-blue-200 text-[12px]">This is what your brief looks like every Monday at 7am.</p>
            <a
              href="#pro-plan-card"
