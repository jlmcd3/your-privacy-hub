  ...
  fixture: {
    insert: {
      ...
      intake_data: {
        ...
        controls: [
          { key: "c1_auth", label: "Authentication", maturity: "Implemented across organisation", notes: "MFA enforced via Okta; SSO for all production systems.", evidence: ["Policy / procedure document", "SOC 2 or auditor letter", "Screenshot / config export"] },
          ...
          { key: "c3_account_access", label: "Account management and access controls", maturity: "Implemented across organisation", notes: "Automated joiner/leaver via Okta + Terraform; least-privilege reviews quarterly.", evidence: ["Policy / procedure document", "Sample log / report"] },
          { key: "c4_inventory", label: "Inventory and management of personal information and systems", maturity: "Implemented across organisation", notes: "Enterprise data map maintained in OneTrust; sensitive-PI inventory reviewed quarterly by CPO and CISO; all production data stores tagged in the CMDB.", evidence: ["Policy / procedure document", "Runbook / SOP"] },
          ...
          { key: "c6_vuln_mgmt", label: "Vulnerability scanning and penetration testing", maturity: "Implemented across organisation", notes: "Snyk + AWS Inspector; annual third-party pen test; critical patches within 7 days.", evidence: ["Policy / procedure document", "Sample log / report", "Third-party pen test / scan report"] },
          { key: "c7_audit_logs", label: "Audit-log management", maturity: "Implemented across organisation", notes: "Centralised Datadog; 13-month retention.", evidence: ["Policy / procedure document", "Sample log / report"] },
          ...
          { key: "c9_anti_malware", label: "Antivirus and anti-malware protections", maturity: "Implemented across organisation", notes: "CrowdStrike on all endpoints.", evidence: ["Policy / procedure document", "Screenshot / config export"] },
          ...
          { key: "c12_awareness", label: "Cybersecurity awareness", maturity: "Implemented across organisation", notes: "Formal enterprise threat-intel programme; membership in Retail & Hospitality ISAC and MS-ISAC; monthly threat-brief circulated to engineering and product; CISO briefs the executive team quarterly.", evidence: ["Policy / procedure document", "Training completion record"] },
          { key: "c13_training", label: "Cybersecurity education and training", maturity: "Implemented across organisation", notes: "Annual training + quarterly phishing simulations for all staff.", evidence: ["Policy / procedure document", "Training completion record"] },
          { key: "c14_secure_dev", label: "Secure development and coding practices", maturity: "Implemented across organisation", notes: "Formal secure SDLC; mandatory code review; SAST (Semgrep) and SCA (Snyk) enforced in CI; annual secure-coding training for engineers.", evidence: ["Policy / procedure document", "Runbook / SOP", "Third-party pen test / scan report"] },
          ...
          { key: "c16_retention", label: "Retention schedules and proper disposal of personal information", maturity: "Implemented across organisation", notes: "Enterprise retention schedule published; 13-month deletion enforced on warehouse; automated lifecycle policies on S3 and Snowflake; quarterly disposal attestations.", evidence: ["Policy / procedure document", "Runbook / SOP", "Sample log / report"] },
  ...
  fixture: {
    insert: {
      ...
      intake_data: {
        ...
        human_review: "Yes — meaningful human review by a trained underwriter with authority to override the model output",
        ...
        notice_how_it_works_method: "Plain-language explanation plus a linked, layered technical description of the model's inputs, outputs, and role in the decision",
        notice_has_alternative_process: "Yes — applicants may request a fully human-reviewed underwriting decision at any time",
        opt_out_exception: "No exception — we provide a full opt-out right",
        opt_out_methods: ["Interactive online form linked from the Pre-use Notice", "Toll-free number staffed during business hours", "Email to privacy@tomorrow4cariboo.example"],
