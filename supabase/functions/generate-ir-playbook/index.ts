...
W3-T4 — THRESHOLD-GATE DISCIPLINE (transfer from ADMT/CYBER): every regime-specific notification duty is gated by the regime's engagement test — HIPAA (§ 160.103 covered-entity or business-associate status), GLBA (§ 6809(3) financial-institution status), GDPR/UK GDPR (Art. 3 establishment / offering / monitoring), state breach statutes (statutory personal-information definition + affected residents). Do NOT emit a bare "you must notify under X" duty without first stating the engagement test AND either applying it to the intake ("the record as documented establishes covered-entity status because …") or framing the duty as conditional ("if the business is a HIPAA covered entity, then …"). Where the engagement test is not answerable from the intake, the duty is CONDITIONAL and information_needed carries the specific fact required to resolve engagement — never assert engagement from a sector label alone.
`,
};
