// ITEM 417 LEG D — shared refinement dependencies for the IR Playbook product.
//
// Mirrors cyber-refinement-deps.ts / governance-refinement-deps.ts /
// admt-refinement-deps.ts / lia-refinement-deps.ts / biometric-refinement-deps.ts
// EXACTLY. The generation model is passed in EXPLICITLY by the caller
...
//
// Metering is identical to the other seven products: one `api_usage` row per
...
export const IR_REFINEMENT_ENABLED = true;

export function makeIrRefinementDeps(
  sourceRowId: string | null,
  /** REQUIRED — the run's resolved generation model. Never defaulted here. */
  generationModel: string,
  fnLabel = "generate-ir-playbook",
): RefinementDeps {
