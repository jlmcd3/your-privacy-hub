...
Return a JSON object with EXACTLY these top-level fields:
{
  ...
  "governance": {
    ...
    "sc_core_activity": "when special_category is Yes, EXACTLY one of: 'Yes — a primary activity, or inextricably part of delivering our principal products or services' | 'No — an ancillary or supporting activity' | 'Uncertain'; else 'n/a'",
    "sc_core_activity_explanation": "string — one sentence on why the special-category processing is primary or ancillary; empty when special_category is No",
    "sc_data_subjects_count": "string — approximate number of individuals a year, e.g. '74,000'; empty when special_category is No",
    "sc_population_proportion": "when special_category is Yes, EXACTLY one of: 'Yes — a significant proportion of the relevant population' | 'No' | 'Unsure'; else 'n/a'",
    "sc_data_volume": "string — the volume and range of special-category data; empty when special_category is No",
    "sc_duration": "when special_category is Yes, EXACTLY one of: 'Continuous or ongoing' | 'Recurring' | 'Long-term but for a fixed period' | 'Temporary or one-off'; else 'n/a'",
    "sc_geographic_scope": "when special_category is Yes, EXACTLY one of: 'Local' | 'National' | 'Several Member States or countries' | 'Broader than the EU/EEA and the UK'; else 'n/a'",
...
Return a JSON object with EXACTLY these fields:
{
  "lia": {
    ...
    "purpose_details": { "interest_holder": "one of EXACTLY: 'Our organisation only' | 'Our organisation and a third party (e.g. business partner)' | 'A third party we share data with' | 'The data subject themselves' | 'The wider public'", "interest_type": "one of EXACTLY: 'Commercial / revenue-related' | 'Operational / service delivery' | 'Security / fraud prevention' | 'Legal / regulatory compliance' | 'Public interest / societal benefit' | 'Research / product improvement' | 'Political / electoral campaigning'", "interest_statement": "string — one full sentence articulating the specific legitimate interest being pursued, stated as live today (never 'we plan to' or 'might')", "specific_benefit": "string — the concrete benefit the processing delivers", "beneficiary": "string — verbatim option: 'Our business' | 'The individuals whose data is processed' | 'A third party' | 'Our business and the individuals' | 'Our business and a third party'", "controller_is_public_authority": "Yes or No", "public_task_processing": "Yes | No | Not applicable ('Not applicable' when the controller is not a public authority)", "device_access": "one of EXACTLY: 'Yes' | 'No' | 'Not sure' — whether the processing stores information on, or reads information from, people's phones, computers or browsers (cookies, pixels, SDK or advertising identifiers, device or browser fingerprinting)", "device_access_strictly_necessary": "ONLY when device_access is 'Yes': one of EXACTLY: 'Yes — all of it is strictly necessary' | 'No — some or all of it goes further' | 'Not sure' (strictly necessary = the requested service cannot be delivered without it; analytics, advertising and personalisation are not); otherwise omit the key" },
...
Return a JSON object with EXACTLY these fields:
{
  ...
  "cppaRisk": {
    ...
    "q7_right_delete": "string", "q8_right_correct": "string", "q9_opt_out": "EXACTLY one of: 'Yes, prominently on homepage' | 'Yes, but in footer only' | 'Yes — in the settings area of our app, smart TV or other device without a homepage' | 'In progress' | 'No'",
  ...
  return {
    ...
    governance: (() => {
      ...
      return {
        ...
        // DOC 258 — Art. 37(1)(c) elements under a "Yes"; hidden values otherwise.
        sc_core_activity: special ? (bucket === "healthcare" ? "Yes — a primary activity, or inextricably part of delivering our principal products or services" as const : "No — an ancillary or supporting activity" as const) : "n/a" as const,
        sc_core_activity_explanation: special ? (bucket === "healthcare" ? "Patient care records are the service delivered." : "The special-category data supports staff access control only.") : "",
        sc_data_subjects_count: special ? (slot === 1 ? "180,000" : "140") : "",
        sc_population_proportion: special ? "No" as const : "n/a" as const,
        sc_data_volume: special ? (bucket === "healthcare" ? "Clinical records with diagnoses and treatment history." : "A biometric template and access timestamps per employee.") : "",
        sc_duration: special ? "Continuous or ongoing" as const : "n/a" as const,
        sc_geographic_scope: special ? (euUk ? "National" as const : "Local" as const) : "n/a" as const,
    ...
    return {
      ...
      lia: {
        ...
        purpose_details: {
          ...
          // DOC 189 — the fallback reads device type for account security:
          // device access claimed strictly necessary.
          device_access: "Yes",
