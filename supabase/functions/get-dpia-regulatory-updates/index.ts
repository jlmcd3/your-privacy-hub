// Surface regulatory updates relevant to a DPIA framework document.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const dataCategories: string[] = intake?.data_categories ?? [];
    const processingType: string = (intake?.processing_activity_type ?? "").toLowerCase();

    const tagSet = new Set<string>(DPIA_TAGS);
    ...
    for (const cat of dataCategories) {
      ...
      for (const [key, tags] of Object.entries(DATA_CAT_TAG_MAP)) {
        if (lc.includes(key)) tags.forEach((t) => tagSet.add(t));
      }
    }
    if (
      processingType.includes("automated") ||
      processingType.includes("ai") ||
      processingType.includes("adm")
    ) {
      tagSet.add("automated-decision-making");
      tagSet.add("gdpr-article-22");
    }
    ...
    console.error("get-dpia-regulatory-updates error:", err);
