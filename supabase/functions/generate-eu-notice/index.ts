...
<p>Where personal data is obtained from a source other than you, we provide this information within a reasonable period after obtaining it (at the latest within one month); or, if the data is used to communicate with you, at the latest at the time of first communication; or, if disclosure to another recipient is envisaged, at the latest when the data is first disclosed (Art. 14(3) of the ${escapeHtml(lawName)}).</p>`,
  ...
  } else if (!isGdprFamily && (collectionSource === "indirect" || collectionSource === "mixed")) {
    // S-N2 (doc 80, 2026-08-27) — the non-GDPR frameworks previously
    // received NO source disclosure at all when data is obtained
    // indirectly. One neutral block: the sources as answered, with no
    // framework-specific timing claims (those belong to the GDPR-family
    // branch above, whose Art. 14(3) rules are that family's law alone).
    const sourceCats = formatAnswer("data_source_categories", answers["data_source_categories"]) || "third-party sources";
    sections.push({
      title: "Where this personal data comes from",
      html: `<p>Where we did not obtain your personal data directly from you, we obtain it from the following sources: ${escapeHtml(sourceCats)}.</p>`,
    });
  }
...
<p>You may request a copy of the safeguards by contacting us at <a href="mailto:${escapeHtml(contactEmail)}">${escapeHtml(contactEmail)}</a>.</p>`,
  ...
  // S-N4 — the Key points bag: ALREADY-FORMATTED reader-label values (this
  // function is the single writer for them); placeholders map to "".
  const keyPoints = {
    controller: controllerName === "[Controller name]" ? "" : controllerName,
    categories: categories === "—" ? "" : categories,
    purposes: purposes === "—" ? "" : purposes,
    basis: lawfulBasis === "—" ? "" : lawfulBasis,
    recipients: recipients === "—" ? "" : recipients,
    transfers: transfersYes,
    retention: retention === "Not specified" ? "" : retention,
    contactEmail: contactEmail === "[contact email]" ? "" : contactEmail,
  };

  return { lawName, controllerName, contactEmail, intro, sections, keyPoints };
}


// S-N5 (doc 80, 2026-08-27) — the required-field screen lives in
// _local/validate.ts (testable without this module's Deno.serve listener);
// re-exported here for API stability.
import { draftBannerHtml, establishmentConsistencyBannerHtml, missingRequiredEuFields } from "./_local/validate.ts";
export { missingRequiredEuFields };
// S-N4 (doc 80, 2026-08-27) — the Key points first layer.
import { buildEuKeyPointsHtml } from "./_local/key-points.ts";
...
${establishmentConsistencyBannerHtml(opts.answers)}
  ...
  try {
    ...
    EdgeRuntime.waitUntil((async () => {
      ...
      try {
        ...
        // S-N5 (doc 80, 2026-08-27) — server-side required-field screen;
        // the documents render the banner, this is the server-side record.
        const missingRequired = missingRequiredEuFields(answers);
        if (missingRequired.length > 0) {
          console.warn(`[generate-eu-notice] required fields missing (banner rendered): ${missingRequired.join(", ")} — session ${sessionId}`);
        }
