// Surface regulatory updates relevant to a generated DPA document.
...
const JURISDICTION_TAG_MAP: Record<string, string[]> = {
  ...
  "UK / UK GDPR": ["uk", "united-kingdom", "ico", "uk-gdpr"],
  "Switzerland": ["switzerland", "fadp", "fdpic"],
  "United States": ["us-federal", "ftc", "us-states"],
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const controllerJ: string = intake?.controllerJurisdiction ?? "";
    const processorJ: string = intake?.processorJurisdiction ?? "";
    const legalFramework: string = intake?.legalFramework ?? "";
    const jurisdictions = [controllerJ, processorJ, legalFramework].filter(Boolean);

    const tagSet = new Set<string>(DPA_TAGS);
    for (const j of jurisdictions) {
      const mapped = JURISDICTION_TAG_MAP[j];
      if (mapped) mapped.forEach((t) => tagSet.add(t));
      else if (j) tagSet.add(j.toLowerCase());
    }
    ...
    console.error("get-dpa-regulatory-updates error:", err);
