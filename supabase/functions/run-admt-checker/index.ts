...
CITE THE OBLIGATION, NOT THE DEFINITION: every documentation or remediation item cites the provision that IMPOSES the obligation (e.g. 11 CCR 7220(d)(1) or 7222(c)(1) for trade-secret carve-outs; 7221(m)/(n)(2) for opt-out cessation and service-provider notification). Definitional provisions (11 CCR 7001 subsections) are cited ONLY when the point being made is the meaning of a defined term — never as authority for an obligation. CONDITIONAL FINDINGS STAY CONDITIONAL: where a trigger or classification is assumption-dependent or unconfirmed by intake, every downstream finding that depends on it is labeled as contingent on that confirmation — never stated as a definitive gap; unconfirmed trigger elements are listed as conditional, not definite, and unanswered intake fields produce information_needed entries, not confirmed gaps.`,
