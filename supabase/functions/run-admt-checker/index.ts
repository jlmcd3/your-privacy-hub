...
IMMEDIATE ACTIONS CARRY A CONSEQUENCES-OF-NON-COMPLIANCE NOTE: the priority_actions list, or the header text immediately preceding it, MUST include a brief note on the potential consequences of non-compliance next to any actions labelled 'IMMEDIATE' — the current list format states actions without stating what happens if they are not taken, which understates urgency. Canonical form of the note (emit once, at the top of priority_actions or as its introductory sentence — never per action, and cross-reference the enforcement-exposure summary rather than restating figures per the ENFORCEMENT EXPOSURE STATED ONCE rule): 'Failure to complete the IMMEDIATE actions below by the compliance deadlines cited (§ 7200(b) — January 1, 2027, for existing ADMT use; § 7155(a)(1) — before initiation, for new processing) exposes the business to CPPA administrative enforcement under Cal. Civ. Code § 1798.155 (per-violation penalties — see the enforcement-exposure summary above), private-right-of-action exposure where a breach involves failure to implement reasonable security under Cal. Civ. Code § 1798.150, and — for continuing violations — daily accrual until remediation is documented.' Do NOT elaborate beyond the note; the per-action deadline remains the operational lever.

ADMT DEFINITION IS DISJUNCTIVE — MATCH § 7001(e) STRUCTURE: when characterising the § 7001(e) ADMT definition in is_admt_reasoning, scope_analysis.summary, or any narrative field, use the enacted disjunctive structure — 'uses computation to replace human decisionmaking OR to substantially replace human decisionmaking' — with the disjunction visible. Do NOT compress the two limbs into 'uses computation to replace or substantially replace human decisionmaking' (the compressed form can be misread as requiring both conditions simultaneously). Preserve the two-limb structure in every restatement, and where a specific fact pattern engages only one limb, name the engaged limb.
