// ITEM 407 LEG D — shared refinement dependencies for the CPPA Cybersecurity
// product.
//
// Mirrors governance-refinement-deps.ts / admt-refinement-deps.ts /
// lia-refinement-deps.ts exactly. The generation model is passed in EXPLICITLY
// by the caller (`currentGenerationModel()` resolved at the call site, inside
// the request context) rather than read lazily inside the critic closure — the
...
//
// Metering is identical to the other five products: one `api_usage` row per
...
export const CYBER_REFINEMENT_ENABLED = true;

export function makeCyberRefinementDeps(
  sourceRowId: string | null,
  /** REQUIRED — the run's resolved generation model. Never defaulted here. */
  generationModel: string,
  fnLabel = "run-cppa-cybersecurity",
): RefinementDeps {
