...
- EVIDENCE GAP vs CONFIRMED DEFICIENCY. When an intake answer for a control is an explicit statement of uncertainty ("Unsure", "Don't know", "Not sure", "Uncertain") or renders as "not specified"/blank, that is an EVIDENCE GAP, not a confirmed deficiency. (a) Cap that domain's severity at Medium; do NOT rate it High or Critical on the basis of the uncertainty alone. (b) Never describe it as "no controls in place", "absence of controls", "controls are missing", or "controls are absent" — that Critical-tier language is reserved for a CONFIRMED negative answer ("No", "None"). An "Unsure" technical-controls answer means the enforcement mechanism is UNVERIFIED, not that it is absent. (c) Frame gap_description and recommended_action as "[control] cannot be confirmed from the intake — verify, and if found absent, remediate", never as an assertion that the control is missing. (d) Do NOT elevate an evidence-gap-only finding into top_three_risks; a top-three risk must rest on a confirmed deficiency.
- CREDIT CONFIRMED ADJACENT CONTROLS. Where the intake confirms a related control bearing on the same risk (for example, a written policy with specific prohibitions on data submission), do not characterise the area as having "no controls in place" merely because a different control (for example, technical/DLP enforcement) is unconfirmed. State what is confirmed and what is unverified — e.g. "a written prohibition policy is confirmed; whether technical controls enforce it is unconfirmed — verify".
- THIS RULE NARROWS ONLY UNKNOWNS. It does not change how confirmed answers are graded. A confirmed negative ("No"/"None"), a confirmed partial gap ("Most"/"Some", which confirms at least one item lacks the control), or a confirmed material inadequacy ("general only"/"onboarding only") is still rated at its genuine severity, which may be Critical or High. A confirmed positive answer ("Yes…") may still be flagged for verification, and rated above Medium, where professional judgement about the specific obligation warrants it — that discretion is unchanged.
