...
export function buildAccountabilityDetermination(
  ...
  // DOC 256 (2026-09-11, batch e2e1185b): the appropriateness clause carries
  // the calibration finding's own first sentence, so the record's risk
  // profile (special categories, scale) is named rather than asserted.
  const calibrationBasis = (() => {
    const first = (riskCalibration.application || "").match(/^[\s\S]{1,320}?[.!?](?=\s|$)/)?.[0]?.trim() ?? "";
    return first ? first.charAt(0).toLowerCase() + first.slice(1).replace(/[.!?]$/, "") : "";
  })();
