// DPA-T6-FIX-TURN (2026-07-26) — deterministic post-pass discharging the
// T6-NONCPPA-MEASUREMENT-BATCH-1 dpa-generator per-tool backlog item 81(b)
// (PRIMARY: unsupported_business_claim ×2 HIGH on quality_run 7650d69c,
// run 90, batch 072eef66-7480-4d33-af19-2e3330050da3).
//
// Mirrors run-li-assessment/_lia_t6_fix.ts (ledger item 89) and
// run-dpia-framework/_dpia_t6_fix.ts (ledger item 92) exactly, adapted to
// the generate-dpa registry + intake. Runs AFTER _w1_dpa_wire and BEFORE
// the LEAK-PREV-P1 emit gate + P2 serializer.
//
...
//   `proposition_key` resolves in DPA_VERIFIED_AUTHORITIES and applies
//   write-around (null pinpoints + write_around=true) for objects on
//   DPA_UNANCHORED_PROPOSITIONS. It does NOT touch objects whose key is
//   unresolved (neither verified nor on the unanchored list) — those retain
//   whatever the model authored. This audit scrubs the pinpoint fields on
...
//   bare "Art."/"Article"/"§"). `citation_misapplied` class is OUT OF SCOPE
//   this turn per dispatch (own future turn).
//
...
//   are downgraded to a neutral "The parties should confirm whether …"
//   sentence when the sentence's content nouns do NOT appear in the
//   flattened intake ledger. Intake-supported claims are preserved verbatim.
//   NEVER emits "information needed" phrasing on customer surfaces
//   (CEO non-CPPA rule: narrow-but-solid; omission over invention).
//
...
export interface DpaT6FixCounters {
  version: string;
  stamp: string;
  build_stamp?: string;
...
// Structural / reserved subtrees — match the W1 wire skip list so the two
// passes agree on what is "customer surface".
...
const ANCHOR_KEYS = new Set<string>([
  ...
  "clause_id", "clause_number", "section_id",
...
const ASSERTIVE_VERB_RE =
  /\b(confirms?|confirmed|shows?|establishes?|established|demonstrates?|proves?|verifies|verified)\b/i;

// DPA voice: "the parties" (Controller + Processor). Matches DPA surface
// conventions (contract prose is written for both parties, not "the
// organisation").
const NEUTRAL_DOWNGRADE =
  "The parties should confirm whether the described position applies here.";
...
const STOPWORDS = new Set<string>([
  ...
  "business","subject","data","processing","assessment","parties","party",
  "agreement","contract","clause","section","dpa",
  "gdpr","article","art","record","records","effective","date",
...
function downgradeString(
  ...
  for (const raw of sentences) {
    ...
    const supported = toks.length === 0 ? false : toks.some(t => intakeText.includes(t));
    if (supported) {
...
function auditNode(node: Record<string, unknown>, c: DpaT6FixCounters): void {
  ...
  if (pk && DPA_VERIFIED_AUTHORITIES[pk] && isVerified) {
 ...
 * Never throws.
 */
export function applyDpaT6Fix(
  report: unknown,
  opts?: { intake?: unknown; buildStamp?: string },
): DpaT6FixCounters {
  const c: DpaT6FixCounters = {
    version: DPA_VERIFIED_AUTHORITY_VERSION,
    stamp: DPA_T6_FIX_STAMP,
