...
interface Body {
  ...
  // authorisation. Absent/unparseable → the standing 30-day default applies.
  subprocessorChangeNoticePeriod?: string | number;
  // S-D7 (doc 80, 2026-08-27) — the Art. 28(2) authorisation-model axis the
  // spec v2 named but the generator hardcoded: "general" (the standing
  // Schedule-1 + notice-and-objection regime) or "specific" (prior specific
  // written authorisation per sub-processor). Absent -> "general", the
  // pre-S-D7 behavior exactly.
  subprocessorAuthorizationModel?: "general" | "specific";
  // S-D2 (doc 80, 2026-08-27) — the structured TOMs intake (PN-D6): ids
  // from DPA_TOMS_TAXONOMY plus optional free-text specifics. Absent ->
  // the pre-S-D2 professional-defaults/placeholder regime unchanged.
  securityMeasuresSelected?: string[];
  securityMeasuresDetails?: string;
...
BREACH NOTIFICATION PARTY RULE: Section 6 governs the Processor's notification obligation to the Controller. Sub-clause 6.2.4 (measures to address the breach) must state: "Describe the measures taken or proposed to be taken by the Processor to address the Personal Data Breach" — NOT "by the Controller." The Controller's own measures are described in its separate Article 33(1) notification to the supervisory authority, which is a distinct instrument. Never write "measures taken or proposed to be taken by the Controller" in the Processor's DPA notification clause.`;
    ...
    // S-D4 (doc 80, 2026-08-27) — anchors moved to _local/registry/dpa-us-citation-anchors.ts (single source of truth, staleness-stamped).
...
- PARTY-FORMATION PLACEHOLDERS (mandatory, overrides the resolved-fact rule below for these two fields only): the record as documented establishes each Party's NAME and its OPERATING jurisdiction only. It does NOT establish either Party's jurisdiction of formation/incorporation or its legal form (entity type). Wherever the document first identifies a Party — the preamble/parties clause, Section 1 (Parties and Recitals), the signature block descriptor, and SCC Annex I.A — render the identification in this exact shape: "<Party name>, a [TO BE COMPLETED: jurisdiction of formation] [TO BE COMPLETED: entity type]" (e.g. "Example Holdings, Inc., a [TO BE COMPLETED: jurisdiction of formation] [TO BE COMPLETED: entity type]"), optionally followed by the operating-jurisdiction fact drawn from the record ("operating in <jurisdiction from the record>"). This applies EQUALLY to the Controller and to the Processor. NEVER write "a Delaware corporation", "a company incorporated under the laws of <operating jurisdiction>", "a limited liability company", "a corporation organised under the laws of …", or any other asserted formation jurisdiction or entity type for either Party — the entity-name suffix (Inc., Ltd., GmbH, LLC, SAS) is NOT evidence of formation jurisdiction or legal form and must not be converted into one. Do not add a separate advisory sentence asking the Parties to confirm formation or legal form; the two placeholders ARE the ask, and each placeholder appears once per Party per identification site.
- PROPORTIONATE ASKS — NO PLACEHOLDER FOR A RESOLVED FACT (R1b2 rule 2b): a fact the PARTIES block supplies with a populated value (controller name, controller jurisdiction, processor name, processor jurisdiction, services description, data categories, retention description, sub-processor presence and named list where supplied, audit-rights level, transfer-clause inclusion flag, transfer mechanism where transfer clause is included, document-type framework) is a RESOLVED input for this run and MUST NOT be re-emitted as a [TO BE COMPLETED: …] placeholder in any clause, schedule, or annex — draft the supplied value into the operative text using the drafting register the surrounding rules require. The Parties' jurisdiction of FORMATION and legal FORM are expressly NOT resolved facts and are governed by PARTY-FORMATION PLACEHOLDERS above. Legitimate [TO BE COMPLETED: …] placeholders are (i) genuinely absent fields the intake structurally does not carry (effective date, notice addresses, principal-agreement identification, categories and approximate volume of data subjects, per-sub-processor location and role where only the list is supplied, per-vendor SCC module selection, per-recipient onward-transfer safeguards, security-control specifics that PLACEHOLDER NEUTRALITY forbids drafting inline, jurisdiction of formation and entity type for each Party), and (ii) fields governed by the ACCOUNTABILITY-MINIMUM EXCEPTION only where that exception applies. This rule sits alongside — not in place of — the PLACEHOLDER NEUTRALITY and ACCOUNTABILITY-MINIMUM EXCEPTION rules above, which are unchanged and continue to govern the SHAPE of any placeholder the drafter does emit; it addresses only the antecedent question of WHEN a placeholder is legitimate at all.
...
9. SUB-PROCESSOR AUTHORISATION MODEL. Use exactly one coherent regime, per the record's selection. ${body.subprocessorAuthorizationModel === "specific" ? `The record selects SPECIFIC authorisation: every engagement or replacement of a sub-processor requires the Controller's prior specific written authorisation obtained before the engagement commences; Schedule 1 lists the sub-processors already so authorised. Do NOT draft a notice-with-objection framework or any general-authorisation language; notice-with-objection and specific authorisation are alternatives under Art. 28(2) and must not be mixed.` : `The record selects (or defaults to) GENERAL authorisation limited to the sub-processors listed in Schedule 1, with new sub-processors permitted only via ${SUBPROC_NOTICE_DAYS}-day advance written notice and a 15-day objection right. Do NOT use the phrase "specific authorisation" anywhere; specific authorisation and notice-with-objection are alternatives under Art. 28(2) and must not be mixed.`}
...
10a. DPO AND REPRESENTATIVE REPRESENTATIONS (S-D6). Include a clause in which each Party represents that it has designated a data protection officer where Article 37 GDPR (or Article 37 UK GDPR) requires one, and that it has appointed an EU representative under Article 27 GDPR or a UK representative under Article 27 UK GDPR where those articles require one. Draft these strictly as where-required representations citing the articles; never assert as a fact that a DPO or representative IS appointed (the record does not establish it), and never demand appointment where the articles do not require it.
...
- ENTITY-FORM OBSERVATIONS ARE SUPERSEDED: the record does not establish either Party's legal form or jurisdiction of formation, so no entity-form observation, suffix analysis, atypicality note, or "the Parties shall confirm the legal form / incorporation jurisdiction" sentence may be emitted anywhere in the document. The PARTY-FORMATION PLACEHOLDERS rule governs exclusively: each Party is identified as "<Party name>, a [TO BE COMPLETED: jurisdiction of formation] [TO BE COMPLETED: entity type]", and those placeholders are the only ask on this subject.
...
17. CLAUSE PARTY-VOICE (R-TURN-3): controller-specific obligations are drafted in the Controller's voice ("Controller shall …"), processor-specific obligations in the Processor's voice ("Processor shall …"), and MUTUAL obligations (cooperation on data-subject rights mechanics, confidentiality, notice-delivery mechanics, dispute-resolution cooperation) in "the Parties shall …" voice. Do NOT collapse one-sided controller or processor duties into generic "the Parties shall …" phrasing — a processor duty under GDPR Art. 28(3)(a)–(h) is a Processor duty and must read "Processor shall …"; a controller duty (documented instructions, lawful-basis representation, controller warranties) must read "Controller shall …". Where a single clause contains both a Controller duty and a corresponding Processor duty (e.g. Controller shall issue instructions; Processor shall process only on instructions), draft each duty in its own party voice within the clause; do not merge into "the Parties shall". "The Parties shall" is reserved for truly bilateral cooperation duties.
18. CONDITIONAL MODULE FAMILY (R-TURN-3, extends REBUILD-DPA T2 SPECULATIVE-CLAUSE BAN): the TARGETED-ADVERTISING module and the MINORS / CHILDREN'S-DATA module are members of the conditional-module family alongside HIPAA/PHI, GLBA/FCRA, and AI/ML-training. Each appears ONLY when the record as documented establishes the corresponding processing fact: targeted-advertising clauses (cross-context behavioural advertising duties, sale/share opt-out plumbing, CCPA/CPRA § 1798.140(ah) "share" implementations, VCDPA/CPA/CTDPA targeted-advertising opt-out) require the record to state that targeted advertising or cross-context behavioural advertising is in scope of the Services; minors / children's-data clauses (COPPA verifiable parental consent, FERPA school-official exception, GDPR Art. 8 age of consent, Recital 38 children safeguards, state-law minors provisions such as CCPA § 1798.120(c) opt-in-to-sell for 13-15) require the record to state processing of children's data or subjects under 18 in scope. Where the record is silent on either fact, the DPA is silent on the corresponding module — no hedged, "in the event", "should the Processor process minors' data", or "to the extent targeted advertising is used" alternative-scenario clauses. Where the record as documented establishes the fact, draft the module cleanly with the specific statutory anchors engaged by the parties' jurisdictions. Speculative inclusion of either module is a HARD violation and will regenerate the document.`;
