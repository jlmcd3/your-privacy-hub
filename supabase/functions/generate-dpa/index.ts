...
- DEFERRED COMPLIANCE-CRITICAL PERIODS CARRY THEIR CONSTRAINT: where a notification or assistance period is deferred to the Parties and a statutory deadline depends on it (e.g. the Processor's obligation to pass on data subject requests, which feeds the Controller's Article 12(3) one-month response deadline), the placeholder states the governing constraint without supplying a value: '[TO BE COMPLETED: notification period — must be short enough to enable the Controller to meet its Article 12(3) one-month response deadline]'. Never a bare '[TO BE COMPLETED: notification period]' where a statutory clock depends on the term, and never a supplied default value.
