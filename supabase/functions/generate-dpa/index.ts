...
EU-CONTROLLER PRIMARY RULE: Identify the controller's jurisdiction from the PARTIES block. If the controller is established in an EU/EEA member state (Germany, France, Ireland, Spain, Italy, Netherlands, Belgium, Sweden, Denmark, Poland, Norway, Portugal, Austria, Finland, Luxembourg, Greece, Switzerland, or any other EU/EEA state), the PRIMARY legal framework is EU GDPR (Regulation (EU) 2016/679). UK GDPR applies IN ADDITION only where the processing also involves UK data subjects or a UK-established party. Do NOT produce a DPA that references only UK GDPR for an EU-established controller. The recitals must cite Regulation (EU) 2016/679 as the operative instrument. If the intake confirms UK data subjects or a UK-established party are involved (not merely possible), add, unambiguously as a present-tense statement of applicability, not a hedge: "This DPA is also governed by the UK General Data Protection Regulation ('UK GDPR') as defined in section 3(10) of the Data Protection Act 2018, to the extent the processing involves data subjects in the United Kingdom." If UK involvement is NOT confirmed by the intake, do not add any UK-GDPR sentence at all — state plainly that the DPA is governed by EU GDPR only, and that UK processing, if it arises later, would require an amendment or addendum. Never use language that could be read as leaving it open whether UK GDPR currently applies (e.g. "where applicable," "to the extent the processing... also involve[s]...") without the intake having actually confirmed UK involvement.
...
- UK IDTA DIRECTIONALITY: do not describe the UK IDTA as governing only transfers FROM the UK. State both directions: 'The UK IDTA (or the UK Addendum to the EU SCCs) may be required for transfers from the EEA to the UK, or for transfers from the UK to a third country; the Parties shall execute the appropriate version and clauses for the direction that applies.'
