...
- GERMAN SUPERVISORY AUTHORITY PRECISION: Germany has 17 state-level DPAs (16 Länder plus the federal BfDI for specific sectors). Do not state 'the competent German supervisory authority' as if there is one national DPA. State: 'The competent supervisory authority for [German entity] depends on its registered seat within Germany; the Parties shall verify and document the applicable state DPA.' If a lead supervisory authority for the overall controller-processor relationship has already been established elsewhere in the document (e.g. CNIL for a French controller), distinguish that from the German sub-processor's own state-DPA registration.
