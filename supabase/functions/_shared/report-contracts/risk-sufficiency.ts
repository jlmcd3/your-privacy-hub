/**
 * ITEM 425 — THE CANONICAL RECORD-SUFFICIENCY RECORD.
 *
 * `record_sufficiency` is tri-shaped in the wild (item 420 inventory):
 *   • legacy `string[]`   — the live LTP array of paragraphs (the R6 litany);
 *   • legacy object       — `{ complete, statement }` (loop2 + index.ts readers);
 *   • bare `string`       — the collapsed single-paragraph shape.
 * ITEM 425 adds the FOURTH and canonical shape:
 *   • typed record        — `{ complete, statement, elements[] }`.
 *
 * This module changes NO writer. It exists so every READER tolerates all four
 * shapes without dropping content (the item-420 idiom, applied to the second
 * re-typed surface). `coerceNarrativeList` is deliberately NOT touched: typed
 * readers get this companion path exactly as `priority_actions` did.
 *
 * Placement rule: multi-consumer by design — it stays in `_shared`.
 * The frontend mirror lives at src/lib/risk-sufficiency.ts (Deno edge code
 * cannot import from src/); a parity test pins the two to identical verdicts.
 */
