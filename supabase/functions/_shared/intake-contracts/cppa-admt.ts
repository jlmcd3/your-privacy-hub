...
// doc 263 run 1 (2026-09-17) — the notice questions (timing, purpose text,
// full text, element texts) are shown once any delivery method other than
// "not yet" is selected (ADMTChecker.tsx, the notice block).
const NOTICE_PROVIDED_DELIVERY: readonly string[] = NOTICE_DELIVERY_OPTIONS.filter(
  (o) => o !== "We have not yet provided a Pre-use Notice",
);
...
export const cppaAdmtContract: IntakeContract = {
  ...
  fields: [
    ...
    { key: "notice_purpose_text",         kind: "narrative", required: "conditional",
      requiredWhen: 'notice_has_specific_purpose is "Yes"',
      trigger: { key: "notice_has_specific_purpose", equals: ["Yes"] }, },
    ...
    { key: "notice_timing", kind: "enum", required: "conditional",
      requiredWhen: 'a delivery method other than "We have not yet provided a Pre-use Notice" is selected (ADMTChecker.tsx: the notice block is hidden on that answer)',
      trigger: { key: "notice_delivery[]", equals: NOTICE_PROVIDED_DELIVERY }, options: NOTICE_TIMING_OPTS },

    { key: "opt_out_exception",             kind: "text",       required: "always" }, // ChoiceWithOther — free-string post-fold; enum list below is advisory
    { key: "opt_out_methods",               kind: "multi-enum", required: "conditional",
      requiredWhen: 'the opt-out path shows the mechanics — full opt-out, or an unresolved "Other" (admtOptOutPath.ts showsOptOutMechanics)',
      trigger: { key: "opt_out_exception", equals: ["No exception — we provide a full opt-out right", "Other — my situation differs (describe)"] }, options: OPT_OUT_METHODS },
    { key: "opt_out_link_title",            kind: "text",       required: "conditional",
      requiredWhen: 'the opt-out path shows the mechanics — full opt-out, or an unresolved "Other" (admtOptOutPath.ts showsOptOutMechanics)',
      trigger: { key: "opt_out_exception", equals: ["No exception — we provide a full opt-out right", "Other — my situation differs (describe)"] }, },
    { key: "opt_out_no_cookie_banner",      kind: "enum",       required: "conditional",
      requiredWhen: 'the opt-out path shows the mechanics — full opt-out, or an unresolved "Other" (admtOptOutPath.ts showsOptOutMechanics)',
      trigger: { key: "opt_out_exception", equals: ["No exception — we provide a full opt-out right", "Other — my situation differs (describe)"] }, options: OPT_OUT_NO_COOKIE_BANNER_OPTS },
    { key: "opt_out_no_account_required",   kind: "enum",       required: "conditional",
      requiredWhen: 'the opt-out path shows the mechanics — full opt-out, or an unresolved "Other" (admtOptOutPath.ts showsOptOutMechanics)',
      trigger: { key: "opt_out_exception", equals: ["No exception — we provide a full opt-out right", "Other — my situation differs (describe)"] }, options: OPT_OUT_NO_ACCOUNT_REQUIRED_OPTS },
    { key: "opt_out_confirmation_mechanism", kind: "text",      required: "conditional",
      requiredWhen: 'the opt-out path shows the mechanics — full opt-out, or an unresolved "Other" (admtOptOutPath.ts showsOptOutMechanics)',
      trigger: { key: "opt_out_exception", equals: ["No exception — we provide a full opt-out right", "Other — my situation differs (describe)"] }, },
    ...
    { key: "opt_out_fairness_doc",          kind: "narrative",  required: "conditional",
      requiredWhen: 'an employment exception (§ 7221(b)(2) hiring/admission or (b)(3) work allocation/compensation) is selected',
    ...
    { key: "opt_out_15_day_process",        kind: "narrative",  required: "conditional",
      requiredWhen: 'the opt-out path shows the mechanics — full opt-out, or an unresolved "Other" (admtOptOutPath.ts showsOptOutMechanics)',
      trigger: { key: "opt_out_exception", equals: ["No exception — we provide a full opt-out right", "Other — my situation differs (describe)"] }, },
    // DOC 158 — § 7221(f), (i), (j), (k), (m) handling duties (full opt-out path).
    { key: "opt_out_handling_confirmations", kind: "multi-enum", required: "conditional",
      requiredWhen: 'the opt-out path shows the mechanics — full opt-out, or an unresolved "Other" (admtOptOutPath.ts showsOptOutMechanics)',
      trigger: { key: "opt_out_exception", equals: ["No exception — we provide a full opt-out right", "Other — my situation differs (describe)"] }, options: OPT_OUT_HANDLING_OPTS },
    ...
    { key: "admt_detail.vendor_status",         kind: "enum",       required: "conditional",
      requiredWhen: 'third_party_admt names a system: non-empty and not an explicit No/None (CEO decision ee860fd0; the vendor block opens only for a named system)',
      trigger: { key: "third_party_admt", present: true, unlessLeadingWord: ["no", "none"] }, options: ADMT_VENDOR_STATUS_OPTS },
    { key: "admt_detail.vendor_docs",           kind: "multi-enum", required: "conditional",
      requiredWhen: 'third_party_admt names a system: non-empty and not an explicit No/None (CEO decision ee860fd0; the vendor block opens only for a named system)',
      trigger: { key: "third_party_admt", present: true, unlessLeadingWord: ["no", "none"] }, options: ADMT_VENDOR_DOCS_OPTS },
    { key: "admt_detail.vendor_makes_available", kind: "enum",      required: "conditional",
      requiredWhen: 'third_party_admt names a system: non-empty and not an explicit No/None (CEO decision ee860fd0; the vendor block opens only for a named system)',
      trigger: { key: "third_party_admt", present: true, unlessLeadingWord: ["no", "none"] }, options: ADMT_YES_NO_UNSURE_OPTS },
    { key: "admt_detail.v_audit",               kind: "enum",       required: "conditional",
      requiredWhen: 'third_party_admt names a system: non-empty and not an explicit No/None (CEO decision ee860fd0; the vendor block opens only for a named system)',
      trigger: { key: "third_party_admt", present: true, unlessLeadingWord: ["no", "none"] }, options: ADMT_YES_NO_OPTS },
    { key: "admt_detail.v_assist",              kind: "enum",       required: "conditional",
      requiredWhen: 'third_party_admt names a system: non-empty and not an explicit No/None (CEO decision ee860fd0; the vendor block opens only for a named system)',
      trigger: { key: "third_party_admt", present: true, unlessLeadingWord: ["no", "none"] }, options: ADMT_YES_NO_OPTS },
    { key: "admt_detail.v_optout",              kind: "enum",       required: "conditional",
      requiredWhen: 'third_party_admt names a system: non-empty and not an explicit No/None (CEO decision ee860fd0; the vendor block opens only for a named system)',
      trigger: { key: "third_party_admt", present: true, unlessLeadingWord: ["no", "none"] }, options: ADMT_YES_NO_OPTS },
    { key: "admt_detail.v_appeal",              kind: "enum",       required: "conditional",
      requiredWhen: 'third_party_admt names a system: non-empty and not an explicit No/None (CEO decision ee860fd0; the vendor block opens only for a named system)',
      trigger: { key: "third_party_admt", present: true, unlessLeadingWord: ["no", "none"] }, options: ADMT_YES_NO_OPTS },
    { key: "admt_detail.v_incident",            kind: "enum",       required: "conditional",
      requiredWhen: 'third_party_admt names a system: non-empty and not an explicit No/None (CEO decision ee860fd0; the vendor block opens only for a named system)',
      trigger: { key: "third_party_admt", present: true, unlessLeadingWord: ["no", "none"] }, options: ADMT_YES_NO_OPTS },
    ...
    { key: "admt_detail.vendor_product",        kind: "text",       required: "conditional",
      requiredWhen: 'third_party_admt names a system: non-empty and not an explicit No/None (CEO decision ee860fd0; the vendor block opens only for a named system)',
      trigger: { key: "third_party_admt", present: true, unlessLeadingWord: ["no", "none"] }, },
    { key: "admt_detail.vendor_training_rights", kind: "text",      required: "conditional",
      requiredWhen: 'third_party_admt names a system: non-empty and not an explicit No/None (CEO decision ee860fd0; the vendor block opens only for a named system)',
      trigger: { key: "third_party_admt", present: true, unlessLeadingWord: ["no", "none"] }, },
    ...
    { key: "notice_element_text", kind: "structured", required: "conditional",
      requiredWhen: 'a delivery method other than "We have not yet provided a Pre-use Notice" is selected (ADMTChecker.tsx: the notice block is hidden on that answer)',
      trigger: { key: "notice_delivery[]", equals: NOTICE_PROVIDED_DELIVERY }, },
    { key: "notice_element_text.purpose",           kind: "narrative", required: "conditional",
      requiredWhen: 'notice_has_specific_purpose is "Yes"',
      trigger: { key: "notice_has_specific_purpose", equals: ["Yes"] }, },
    { key: "notice_element_text.optout",            kind: "narrative", required: "conditional",
      requiredWhen: 'notice_has_opt_out_desc is not "No" (the notice carries opt-out or exception wording)',
      trigger: { key: "notice_has_opt_out_desc", equals: ["Yes — with specific opt-out instructions", "Mentions opt-out but without clear instructions", "We rely on an exception and describe appeal rights instead", "We rely on an exception and the notice identifies the specific exception"] }, },
    { key: "notice_element_text.access",            kind: "narrative", required: "conditional",
      requiredWhen: 'notice_has_access_desc is "Yes"',
      trigger: { key: "notice_has_access_desc", equals: ["Yes"] }, },
    { key: "notice_element_text.antiretaliation",   kind: "narrative", required: "conditional",
      requiredWhen: 'notice_has_anti_retaliation is "Yes"',
      trigger: { key: "notice_has_anti_retaliation", equals: ["Yes"] }, },
    { key: "notice_element_text.howworks_inputs",   kind: "narrative", required: "conditional",
      requiredWhen: 'notice_has_how_it_works starts with "Yes"',
      trigger: { key: "notice_has_how_it_works", equals: ["Yes — included inline in the notice", "Yes — via hyperlink or layered notice"] }, },
    { key: "notice_element_text.howworks_output",   kind: "narrative", required: "conditional",
      requiredWhen: 'notice_has_how_it_works starts with "Yes"',
      trigger: { key: "notice_has_how_it_works", equals: ["Yes — included inline in the notice", "Yes — via hyperlink or layered notice"] }, },
    { key: "notice_element_text.altprocess",        kind: "narrative", required: "conditional",
      requiredWhen: 'notice_has_alternative_process is "Yes"',
      trigger: { key: "notice_has_alternative_process", equals: ["Yes"] }, },
    ...
    { key: "admt_detail.appeal_reviewer_role",      kind: "text", required: "conditional",
    ...
    { key: "admt_detail.appeal_trained",            kind: "enum", required: "conditional",
      requiredWhen: 'opt_out_exception is the human-appeal exception (§ 7221(b)(1))',
      trigger: { key: "opt_out_exception", equals: ["Human appeal exception (§ 7221(b)(1)) — we provide a human reviewer with authority to overturn the decision"] }, options: ADMT_YES_NO_OPTS },
    { key: "admt_detail.appeal_authority_overturn", kind: "enum", required: "conditional",
      requiredWhen: 'opt_out_exception is the human-appeal exception (§ 7221(b)(1))',
      trigger: { key: "opt_out_exception", equals: ["Human appeal exception (§ 7221(b)(1)) — we provide a human reviewer with authority to overturn the decision"] }, options: ADMT_YES_NO_OPTS },
    { key: "admt_detail.appeal_step_count",         kind: "text", required: "conditional",
      requiredWhen: 'opt_out_exception is the human-appeal exception (§ 7221(b)(1))',
      trigger: { key: "opt_out_exception", equals: ["Human appeal exception (§ 7221(b)(1)) — we provide a human reviewer with authority to overturn the decision"] }, },
    ...
    { key: "admt_detail.sole_use_attestation",      kind: "enum", required: "conditional",
      requiredWhen: 'an employment exception (§ 7221(b)(2) hiring/admission or (b)(3) work allocation/compensation) is selected',
      trigger: { key: "opt_out_exception", equals: ["Hiring/admission exception (§ 7221(b)(2)) — ADMT used solely to assess ability; no unlawful discrimination", "Work allocation/compensation exception (§ 7221(b)(3)) — ADMT used solely for allocation/compensation; no unlawful discrimination"] }, options: ADMT_SOLE_USE_ATTESTATION_ALL_OPTS },
    { key: "admt_detail.nondiscrimination_testing", kind: "enum", required: "conditional",
      requiredWhen: 'an employment exception (§ 7221(b)(2) hiring/admission or (b)(3) work allocation/compensation) is selected',
      trigger: { key: "opt_out_exception", equals: ["Hiring/admission exception (§ 7221(b)(2)) — ADMT used solely to assess ability; no unlawful discrimination", "Work allocation/compensation exception (§ 7221(b)(3)) — ADMT used solely for allocation/compensation; no unlawful discrimination"] }, options: ADMT_NONDISCRIM_TESTING_OPTS },
    ...
    { key: "notice_full_text", kind: "narrative", required: "conditional",
      requiredWhen: 'a delivery method other than "We have not yet provided a Pre-use Notice" is selected (ADMTChecker.tsx: the notice block is hidden on that answer)',
      trigger: { key: "notice_delivery[]", equals: NOTICE_PROVIDED_DELIVERY }, },
