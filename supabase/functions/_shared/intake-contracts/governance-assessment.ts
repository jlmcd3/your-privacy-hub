...
export const governanceContract: IntakeContract = {
  ...
  fields: [
    ...
    // Panel completeness (2026-09-16) — the three detail narratives sit inside
    // the special-category branch (the page emits "" when special_category is
    // not "Yes"), so they are asked only there.
    { key: "sc_core_activity_explanation", kind: "narrative", required: "conditional",
      requiredWhen: 'special_category === "Yes"', trigger: { key: "special_category", equals: ["Yes"] }, hiddenValue: "" },
    { key: "sc_data_subjects_count", kind: "text", required: "conditional",
      requiredWhen: 'special_category === "Yes"', trigger: { key: "special_category", equals: ["Yes"] }, hiddenValue: "" },
    ...
    { key: "sc_data_volume", kind: "narrative", required: "conditional",
      requiredWhen: 'special_category === "Yes"', trigger: { key: "special_category", equals: ["Yes"] }, hiddenValue: "" },
    ...
    { key: "privacy_notice_coverage", kind: "enum", required: "conditional",
      ...
      trigger: { key: "privacy_policy", equals: ["Yes, current (reviewed in last 12 months)", "Yes, but outdated"] },
    ...
    { key: "dpo_status", kind: "enum", required: "conditional",
      requiredWhen: 'eu_uk_data === "Yes" (the page also asks it when an EU/UK jurisdiction or an Art. 3 fact is selected; the gate counts the residents answer)',
      trigger: { key: "eu_uk_data", equals: ["Yes"] },
    ...
    // Panel completeness (2026-09-16) — asked exactly where the page shows them:
    // the count once the contract status is answered; the uncovered list only
    // when coverage is less than all vendors; the routes only for a "Yes".
    { key: "processor_count", kind: "text", required: "conditional",
      requiredWhen: 'dpa_status is answered', trigger: { key: "dpa_status", equals: [...DPA_STATUS] }, hiddenValue: "" },
    { key: "uncovered_vendors", kind: "narrative", required: "conditional",
      requiredWhen: 'dpa_status is not "Yes, all vendors"',
      trigger: { key: "dpa_status", equals: ["Most vendors", "Some vendors", "No"] }, hiddenValue: "" },
    ...
    { key: "transfer_routes", kind: "narrative", required: "conditional",
      requiredWhen: 'transfer_status starts with "Yes"',
      trigger: { key: "transfer_status", equals: ["Yes, US-based tools", "Yes, other non-adequate countries", "Yes, only to countries with an adequacy decision or regulations", "Yes, a mix of routes (described below)"] },
      hiddenValue: "" },
    ...
    { key: "technical_controls_list", kind: "multi-enum", required: "conditional",
      requiredWhen: 'technical_controls is "Yes — DLP/content filtering actively enforced" or starts with "Partial"',
      trigger: { key: "technical_controls", equals: ["Yes — DLP/content filtering actively enforced", "Partial — some tools or categories"] },
    ...
    { key: "dsr_rights_tested", kind: "multi-enum", required: "conditional",
      ...
      trigger: { key: "dsr_capability", equals: ["Yes — documented and tested across all vendors"] },
    ...
    { key: "dpia_ai_coverage", kind: "enum", required: "conditional",
      ...
      trigger: { key: "dpia_status", equals: ["Yes, multiple DPIAs completed", "Yes, one DPIA completed"] },
    ...
    { key: "training_ai_coverage", kind: "enum", required: "conditional",
      ...
      trigger: { key: "training_status", equals: ["Yes, formal onboarding + annual refresh", "Yes, onboarding only"] },
    ...
    { key: "dpa_art28_verified", kind: "enum", required: "conditional",
      requiredWhen: 'dpa_status is not "No" (any agreement exists; Governance master review 2026-09-15, F08)',
      trigger: { key: "dpa_status", equals: ["Yes, all vendors", "Most vendors", "Some vendors"] },
