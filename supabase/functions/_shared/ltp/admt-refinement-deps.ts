// ITEM 395 LEG D — shared refinement dependencies for the CPPA ADMT product.
//
// Mirrors lia-refinement-deps.ts exactly. The generation model is passed in
// EXPLICITLY by the caller (`currentGenerationModel()` resolved at the call
// site, inside the request context) rather than read lazily inside the critic
// closure — the resurrection-bug class: a closure that resolves the model later
// can fall out of the request's AsyncLocalStorage context and silently default
// to sonnet.
//
// Metering is identical to the other three products: one `api_usage` row per
...
export const ADMT_REFINEMENT_ENABLED = true;

export function makeAdmtRefinementDeps(
  sourceRowId: string | null,
  /** REQUIRED — the run's resolved generation model. Never defaulted here. */
  generationModel: string,
  fnLabel = "run-admt-checker",
): RefinementDeps {
