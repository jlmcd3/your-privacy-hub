// ITEM 386 LEG 3 — shared refinement dependencies for the LIA.
//
// Mirrors risk-refinement-deps.ts exactly. The only LIA-specific departure is
// that the generation model is passed in EXPLICITLY by the caller
// (`resolveGenerationModel` / `currentGenerationModel()` at the call site)
// rather than read lazily inside the critic closure — the resurrection-bug
// class: a closure that resolves the model later can fall out of the request's
// AsyncLocalStorage context and silently default to sonnet.
//
