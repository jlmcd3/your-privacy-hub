...
DOC 259A §5 — notice_delivery's "We have not yet provided a Pre-use Notice" option is EXCLUSIVE: never combine it with any actual delivery channel in the same array.`,
...
2026-08-25 batch generator fix — a6_safeguards[] carries TWO adjacent, easily-conflated enums about the SAME safeguard: safeguard_status is whether it is IMPLEMENTED ("Implemented, not tested" is a valid safeguard_status value, e.g.), while effectiveness_basis is separately whether its EFFECTIVENESS has been VALIDATED ("Validated by testing against the linked risk" / "Consistent with an industry standard or framework" / "Based on internal design review only" / "No effectiveness evidence"). Never write a safeguard_status value into effectiveness_basis or vice versa — use the exact options list the contract block above gives for each field, and check which field you are filling before writing "tested"/"not tested" language, since that phrasing belongs to safeguard_status, not effectiveness_basis.
2026-08-26 (TURN 1c) — sensitive_location_basis is a direct Yes/No answer to "does the automated processing derive a personal attribute of a consumer FROM their detected presence at a sensitive location" (11 CCR § 7150(b)(5)). Answer "Yes" ONLY when the scenario's own narrative describes an actual presence-based inference mechanism (e.g. geolocation/beacon/check-in signals used to infer something about the person). Answer "No" for every scenario where the business merely operates in, or processes data sourced from, a sensitive-location-type sector (healthcare, education, religious, etc.) without describing any presence-detection-and-inference activity — a healthcare SaaS vendor scoring clinical records a hospital already collected is "No", not "Yes", even though its data and sector are health-related. Do not treat sector alone as a proxy for this trigger.
2026-08-26 (TURN 1d) — q5b_profiling_observation is likewise a direct Yes/No: "Yes" ONLY when the scenario's narrative describes automated processing that INFERS a characteristic (performance, reliability, health, behavior, etc.) of workers, students, or applicants FROM systematic observation of them (productivity scoring, keystroke analytics, telematics-based driver scoring, location-pattern scoring). Bare monitoring or record-keeping with nothing inferred from it is "No". Neither this field nor sensitive_location_basis is ever answered from sector alone, and neither substitutes for the other — they are separate § 7150(b)(4) and § 7150(b)(5) triggers.
2026-08-27 (batch 3e9ad759) — § 7150(b)(4)'s "applicants" means the business's OWN job applicants or educational-program applicants (the statute's capacity list: educational program applicant, job applicant, student, employee, or independent contractor FOR THE BUSINESS). Consumers applying for a loan, account, insurance policy, or service are NOT this trigger, however systematically they are observed — a FinTech credit-scoring scenario whose affected consumers are borrowers/loan applicants answers q5b_profiling_observation "No" (its triggers are (b)(1)/(b)(3), not (b)(4)). Answer "Yes" only where the observed people stand in an employment or educational relationship to the business itself.`,
...
2026-08-25 C1.2/FC-L4 fields — profile.q1_revenue/q2_consumers/q5_sell_share/q5c_share_revenue_50pct/q15_sensitive_pi/q15c_spi_volume (§ 7120(a)-(b) applicability predicate) and profile.password_auth_used (Yes/No — does the business's authentication include passwords/passphrases at all) are all optional but should be populated on MOST scenarios so the applicability table and the c1_auth predicate resolve determinately rather than "insufficient information". Set password_auth_used consistently with whatever the c1_auth control's own notes describe — "Yes" when notes mention a password/passphrase factor (even alongside MFA), "No" when notes explicitly describe passwordless/FIDO2-only/WebAuthn-only authentication with no password fallback. Leave a minority of scenarios without these fields to exercise the indeterminate branch.`,
...
DOC 259A §5.1 — whenever reasons_to_conduct includes an "Evaluation or scoring", "Automated decision-making", or "Systematic, extensive evaluation" reason, automated_decision_nature is REQUIRED and must be consistent with the record's own narrative (e.g. nature_scope_context): the solely-automated option only where no person reviews the decision before it takes effect, the meaningful-human-review option only where a person with authority actually reviews and can change the outcome BEFORE it takes effect (not a post-hoc appeal), and the no-legal-effect option only where the record genuinely describes no legal or similarly significant effect. Vary all three across scenarios that trigger the question; omit the key entirely on scenarios whose reasons_to_conduct does not trigger it.
QB-P6 — narrative fields must name tools, owners, and dates.
8F/8H — populate the sign-off block (dpia_prepared_by, dpia_approved_by_name, dpia_approved_by_title, dpia_approval_date, dpia_signoff_basis) on MOST scenarios; leave it blank on a minority to exercise the no-approver branch. Give roughly half the scenarios intra-EEA/intra-UK-only transfer flows and the other half at least one genuine third-country destination outside the origin regime, so the third-country risk trigger is exercised both ways. residual_risks is the company's own residual-risk account in its own words and may cite article numbers — but ONLY as the literal word "Article" followed by the number (e.g. "Article 15", "Article 35(7)"); NEVER the "§" section-symbol form anywhere in this scenario, even for an EU/German-flavored record — the fixture-lint statute-cite allowlist only recognizes GDPR cites spelled out as "Article N" and rejects a bare "§ N". THIS RULE APPLIES TO EVERY FIELD IN THE INTAKE, NOT ONLY residual_risks (QB-REPAIR-2, 2026-08-27: live batch 510a9953 aborted DPIA 4/4 on "statute cite outside allowlist: §17"/"§10" written into necessity_proportionality/reasons_to_conduct/codes_of_conduct while naming a legal-obligation instrument under hard constraint 8 below) — when naming a Member State or foreign statute anywhere (necessity_proportionality, nature_scope_context, reasons_to_conduct, codes_of_conduct, or any other narrative field), spell it out as "Section N" or "Article N" of the named act (e.g. "Section 26 of the Bundesdatenschutzgesetz (BDSG)"), never as a bare "§ N".
2026-08-25 batch generator fix — jurisdictions is a CLOSED multi-enum of REGIME-level labels only (exactly the options the contract block above lists for this field, e.g. "EU (GDPR)", "United Kingdom (UK GDPR)") — it is NOT a country-name field. Never write an individual EU member-state name here (no "Germany", "France", "Ireland", etc., even though those exact strings ARE valid values for OTHER tools' jurisdiction-shaped fields like dpa-generator's or ir-playbook's). A record about a German subsidiary still selects "EU (GDPR)" for this field; name the specific country, if at all, only in a narrative/prose field.
...
DOC 259A §5.3 — whenever ai_high_risk is true, ai_annex_iii_point_2 is REQUIRED: "yes" only where the scenario specifically describes the high-risk system as a safety component of digital infrastructure, road traffic management, or the supply of water, gas, heating or electricity (Annex III, point 2); "no" for every other high-risk scenario (employment, credit, biometrics, education, law enforcement, etc.); include at least one "yes" scenario across the batch so that branch is exercised.
...
const RUBRIC_GENERAL: RubricCheck[] = [
  ...
  { id: "rubric_generic_boilerplate",       dimension: "analysis",      severity: "medium",
    description: "Reasoning is generic boilerplate that could apply to any company; not tailored to THIS intake's facts. A sentence that correctly names the SPECIFIC facts the intake supplied and states which legal test those facts satisfy (e.g., naming the actual elements of a multi-part test the record establishes) is NOT boilerplate merely because it uses the test's own standard vocabulary to do so — genericness means the reasoning ignores or could ignore this intake's specific facts, not that the sentence's phrasing is reusable across records that happen to satisfy the same test the same way. Fail this check only when the sentence would read identically regardless of what the company actually reported. DOC 152 (2026-09-03): a report's RATIFIED FIXED FRAMEWORK PROSE — a methodology section stating the product's decision steps (e.g., the CPPA Risk five-step 'How This Assessment Decides'), governing-requirement recitals of statutory text, standing source-class considerations (e.g., automatically-collected or third-party-sourced information carrying stated notice/accuracy considerations), and fixed conclusion frames whose operands vary ('The necessity analysis is qualified: N elements…') — is identical across reports BY DESIGN (CEO-ratified spine and engine frames). Judge tailoring in the company-specific determinations, quoted bases, and per-element analysis those frames introduce; never fail this check on the framework prose itself. DOC 153 (2026-09-03): the same rule covers, by name, the § 2.A customer-voice lead-in ('…is recorded below and is quoted as given'), Section 1 'How This Assessment Decides' (Steps 1–5 and 1.C Qualitative Refinement), and the fixed disposition frames ('the Activity may proceed while the conditions in § 4.D are completed; none requires suspending the processing') — a finding quoting only such a frame, with no company-specific operand identified as untailored, is not a finding." },
  { id: "rubric_unsupported_business_claim", dimension: "hallucination", severity: "high",
    description: "Document asserts facts about the business that are not in the intake (invented users, revenue, jurisdictions, etc.). DOC 245 (2026-09-09): a finding under this check MUST quote the specific business fact that is absent from the intake. A sentence that states two intake facts that conflict and resolves them ('the recorded date controls', 'reconciling the two appears among the Follow-Ups in § 4.D'), or that restates a structured per-row answer with attribution (a recipient's contract status, a safeguard's recorded status), is the report DISCLOSING the record, not asserting beyond it — never fail this check on it." },
  { id: "rubric_actionability",             dimension: "intelligence",  severity: "medium",
    description: "Recommendations are not actionable for a real compliance professional (vague, no owner, no trigger). A report that correctly reaches a determination calling for no further action (e.g., a valid out-of-scope / not-required conclusion, or a fully-approved determination with no open condition) is NOT under-actionable for stopping there once it states that conclusion — judge actionability against what the determination itself calls for, not against unrelated operational detail the intake happened to supply that the determination correctly renders moot (e.g., notice text, opt-out mechanics, vendor documentation supplied for a system later found out of scope). A report is also NOT under-actionable for naming WHO accepted or must act on a risk (e.g., \"preliminary until R. Steiner re-scores it\") without inventing a re-scoring deadline, procedure, or trigger the intake never supplied — this fleet's products never invent unattributed operational detail (deadlines, procedures, named owners for specific mitigation steps) beyond what the record states; naming the accountable person, without more, is the complete and correct behavior. Fail this check only when the report's OWN stated next steps, conditions, or recommendations are themselves vague or unassigned relative to what the intake actually supports — not because a correct, complete determination or a properly-attributed acceptance left unsupported detail unaddressed. DOC 137 (2026-09-02): a report is also NOT under-actionable for explicitly stating that the Company needs to DESIGNATE an owner, a register-maintainer, or a decision-maker for an open item when the intake records no such name or role — e.g. \"The Company will need to designate an owner for resolving this open item\", \"...for the items in this Readiness Action Register; none is currently recorded\", or \"naming the register and the person who maintains it would close the point.\" This is the same no-fabrication design as the acceptance-naming sentence above, applied to the reverse case: the report cannot name a person or role the record never supplied, but flagging the gap and calling for a designation is MORE actionable than staying silent about it, not less. A shared PORTFOLIO-WIDE DEFAULT that IS on record (e.g., a standing \"Group Data Protection Officer\" recorded as the default owner/validation-method/target-date for every remediation item, with that fact disclosed as a default rather than presented as item-specific) is likewise not under-actionable merely for reusing that same default across multiple items — an honestly-disclosed default is a real, attributable answer, not a vague one. Fail this check only when the report neither names an accountable person/role NOR flags that one still needs to be designated, or when a shared default is presented as if it were item-specific analysis rather than disclosed as a default. DOC 152 (2026-09-03): the same no-fabrication design applies to CONDITIONS and FOLLOW-UPS — they state the closure requirement the record supports; the fleet never invents a deadline, milestone, or schedule the intake did not supply, so a condition without an invented deadline is complete, not under-actionable." },
...
const RUBRIC_RISK: RubricCheck[] = [
  ...
  // DOC 229 §3.2 item 1 / DOC 231 — V3 hook citations (dark; RISK_HOOKS_ENABLED).
  { id: "rubric_risk_v3_hook_citations_are_ratified", dimension: "hallucination", severity: "low",
    description: "V3 hook citations in the persuasive-authority-hooks surface are RATIFIED sentences derived from the authority_hooks table, not generated by the LLM. Each sentence citing a named GDPR decision (AENA, Deliveroo, Amazon, Cartonajes, ICS, Poste Italiane, Comune di Bolzano, or any future ratified hook source) or a CPPA Final Statement of Reasons position is a verified regulator authority and is NOT a hallucination. Never score these as invented citations. The full authority label and decision date in each sentence are the CEO-ratified `authority_label` field — check that it matches a known ratified hook, not that it matches your own knowledge of the decision." },
  // DOC 229 §3.2 item 2 / DOC 231 — V3 information_needed from hook disagreement.
  { id: "rubric_risk_v3_information_needed_roo_is_designed", dimension: "hallucination", severity: "low",
    description: "When a V3 hook's two-leg selection disagreed (one leg said 'same', the other 'different') or both said 'unknown', the hook selection records an unsettled entry (a Residual Open Observation) noting that the assessment could not settle a fact comparison against a regulator action from the answer given. This is NOT a gap in analysis — it is the designed output of an honest disagreement signal. Never score a ROO entry asking the customer to describe an answer more specifically, or noting no applicable regulator action was found, as rubric_actionability boilerplate or rubric_generic_boilerplate." },
];
