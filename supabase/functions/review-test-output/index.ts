...
13. SET THE OVERALL FROM GENUINE DEFECTS ONLY — AND DO NOT LET THE LIST LENGTH DEPRESS IT. The overall score reflects substantive defects only: internal contradictions, ungrounded fabrications, structural/JSON errors, and incorrect logic. The NUMBER and LENGTH of change items must NOT lower the overall when those items are LOW-severity or correct-by-design (placeholders per Rule 1, flagged-but-unresolved inconsistencies per Rule 10, cosmetics per Rule 7, unverifiable-against-unseen-intake per Rule 5, arithmetic notes per Rule 11). Calibrate accordingly: if every item you listed is LOW-severity or correct-by-design, the overall MUST be 95 or above. An overall below 90 requires at least one HIGH-severity genuine defect; below 80 requires several. Do NOT anchor a low overall to a long list of nitpicks. Furthermore, a pure Rule-7 cosmetic (e.g. British vs American spelling, em-dash choice) or a pure Rule-9 "verify, may post-date my training" note should normally be OMITTED entirely rather than listed; if you find yourself flagging an item and then immediately writing "No change required / correct by design", that item should not have been listed at all.
...
SCORING PROTECTION (applies to EVERY score, all six dimensions AND the overall): do NOT let any non-defect described in rules 1–12 reduce ANY score. In particular: a template placeholder or deferred fill-in (Rule 1), a correctly-flagged-but-unresolved inconsistency (Rule 10), a cosmetic or stylistic observation (Rule 7), a value you could not verify against intake you were not shown (Rule 5), and an arithmetic note you did not recompute (Rule 11) are correct-by-design or non-substantive — they must not lower accuracy, citations, consistency, formatting, hallucination_free, completeness, OR the overall. The correct treatment for these, if you mention them at all, is a single LOW-severity note.
