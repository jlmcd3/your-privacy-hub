...
export const CPPA_RISK_TOOL_MODULE: ToolModule = {
  ...
  extraRules: [
    ...
    "CANONICAL RECORD REFERENCE (FF-2 T2 — cross-tool CEO ruling D3 as superseded by the v3 CEO-ratified counsel register): in user-facing prose, attribute the business's facts to the business (e.g. 'the business has indicated …', 'the business has described …', 'the record as documented does not yet resolve …'). The 'the company has indicated' family ('the company has indicated', 'the record as documented reflects', 'the record as documented indicates', 'the record as documented demonstrates', 'the record as documented establishes', 'on the record as documented') is BANNED in user-facing prose; 'on the record as documented' is the permitted locative form. NEVER write 'the intake states', 'the intake records', 'the submission', 'the form', or 'the questionnaire' in user-facing prose. Raw intake field ids remain permitted only in information_needed.field / source_fields anchors, per the INTERNAL-VOCAB CLASS BAN.",
    ...
    "INTAKE 'NO' IS AN ANSWER, NOT A SILENCE: distinguish 'the intake affirmatively records No' from 'the intake is silent'. A field recorded false/No (e.g. dpo_or_privacy_officer: false) is DOCUMENTED — as an absence — and is described as such ('the business has indicated it has no designated DPO'), never as 'not documented in the intake' or 'a documentation gap, not a confirmed finding of absence', which is circular. Where the intake is genuinely silent, say the intake does not address the point and route it through the record-completion actions. Never blend the two framings in one finding.",
    ...
    "UNDOCUMENTED IS NOT CONTRADICTORY: where two intake values could be complementary (e.g. a 90-day retention for one data category and a 24-month general period), describe the RELATIONSHIP as undocumented and flag it for the user to specify — do not assert direct contradiction unless the values cannot coexist. Per the PROSE BLACKLIST, do NOT use 'insufficient basis' language in the conclusion; instead use the advocate-drafter voice: name what the record as documented establishes, then name the specific item whose completion would let the determination be recorded as established.",
    ...
    "CYBERSECURITY-AUDIT RATIONALE — NO UNGROUNDED HEDGES, NO CANNED CONCLUSIONS: cross_tool_recommendations.cybersecurity_audit_rationale states a determination grounded in the intake, not a hedged possibility unmoored from the record — and never a conclusion the record cannot support. Read every prong from the injected TEST-STATES block and defer to it (GRADER-1 T6b: state the conclusion the computed state requires, in human phrasing — NEVER the state token or the test id in user-facing prose): (i) sensitive-PI prong — state the conclusion the computed state requires (when the business has indicated no sensitive-PI processing, state that the § 7120(b)(2)(B) sensitive-PI threshold does not apply on the current record; when the business has indicated SPI volume at 50,000 or more, state that the threshold is met on the current record; when the business has indicated SPI volume below 50,000, state that the threshold is not met on the current record; otherwise use advocate-drafter voice: state what the recorded facts establish, then add 'the record does not yet resolve the § 7120(b)(2)(B) 50,000-SPI threshold; recording the exact sensitive-PI volume figure completes the determination.'). Do NOT emit hedges of the form 'may approach the 50,000 sensitive-PI threshold… but this has not been fully confirmed' when the computed state is RESOLVED. (ii) volume prong — state the conclusion the M3 computed state requires, per the BAND-VS-THRESHOLD RULE. R-TURN-2 BELOW-BAND EXEMPLAR (crisp form): where the business has indicated a consumer/household band lying entirely below the § 7120(b)(2)(A) 250,000 threshold (e.g. 100,000–249,999), state the conclusion in one sentence — 'The recorded 100,000–249,999 consumer/household band lies entirely below the § 7120(b)(2)(A) 250,000 threshold, so the volume prong is not met on the current record.' Do NOT re-derive the threshold, do NOT walk through adjacent bands, do NOT hedge with 'approaches' or 'is close to' — the band-below-threshold determination is dispositive and stated in one sentence. (iii) revenue prong (§ 7120(b)(1) 50%-from-sale/share) — state the conclusion the M5 computed state requires (not-met when the business has indicated that personal information is not sold or shared, or that the share of annual revenue derived from selling or sharing is not more than 50%; met when the business has indicated an affirmative answer that more than 50% of annual revenue derives from selling or sharing personal information; otherwise indeterminate with an advocate-drafter completion note). Zero canned prong-specific phrasing beyond what the computed state supports.",
