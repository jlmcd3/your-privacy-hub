...
export const CPPA_RISK_TOOL_MODULE: ToolModule = {
  ...
  extraRules: [
    ...
    "CYBERSECURITY-AUDIT RATIONALE — NO UNGROUNDED HEDGES: cross_tool_recommendations.cybersecurity_audit_rationale states a determination grounded in the intake, not a hedged possibility unmoored from the record. Where normalised_intake.triggers.sensitive_pi_beyond_enumerated is false AND no other intake field affirmatively indicates sensitive-PI processing at or approaching the 50,000-consumer threshold, do NOT write hedges of the form 'may approach the 50,000 sensitive-PI threshold… but this has not been fully confirmed' — state plainly: 'On the current record, sensitive-PI processing is not indicated (triggers.sensitive_pi_beyond_enumerated is false), so the § 7120(b)(2)(B) sensitive-PI threshold does not apply; the § 7120(b)(2)(A) 250,000-consumer volume threshold is not met on the annual_consumer_volume record; and the § 7120(b)(1) 50%-revenue-from-sale/share alternative is [assessed against the intake — state met/not met/insufficient data as applicable].' Where a SPECIFIC intake field DOES suggest sensitive-PI processing (name the field, e.g. q15_sensitive_pi = 'Yes', or a pi_categories entry naming a § 7014 sensitive category), cite that field explicitly in the rationale — never hedge without naming the grounding field.",
