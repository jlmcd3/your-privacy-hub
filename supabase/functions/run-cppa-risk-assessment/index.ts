...
export const CPPA_RISK_TOOL_MODULE: ToolModule = {
  ...
  extraRules: [
    ...
    "CYBERSECURITY-AUDIT RATIONALE — NO UNGROUNDED HEDGES, NO CANNED CONCLUSIONS: cross_tool_recommendations.cybersecurity_audit_rationale states a determination grounded in the intake, not a hedged possibility unmoored from the record — and never a conclusion the record cannot support. Read every prong from the injected TEST-STATES block and defer to it (GRADER-1 T6b: state the conclusion the computed state requires, in human phrasing — NEVER the state token or the test id in user-facing prose): (i) sensitive-PI prong — state the conclusion the computed state requires (when the intake records no sensitive-PI processing, state that the § 7120(b)(2)(B) sensitive-PI threshold does not apply on the current record; when the intake records SPI volume at 50,000 or more, state that the threshold is met on the current record; when the intake records SPI volume below 50,000, state that the threshold is not met on the current record; otherwise use advocate-drafter voice: state what the recorded facts establish, then add 'the record does not yet resolve the § 7120(b)(2)(B) 50,000-SPI threshold; recording the exact sensitive-PI volume figure completes the determination.'). Do NOT emit hedges of the form 'may approach the 50,000 sensitive-PI threshold… but this has not been fully confirmed' when the computed state is RESOLVED. (ii) volume prong — state the conclusion the M3 computed state requires, per the BAND-VS-THRESHOLD RULE. (iii) revenue prong (§ 7120(b)(1) 50%-from-sale/share) — state the conclusion the M5 computed state requires (not-met when the intake records that personal information is not sold or shared, or that the share of annual revenue derived from selling or sharing is not more than 50%; met when the intake records an affirmative answer that more than 50% of annual revenue derives from selling or sharing personal information; otherwise indeterminate with an advocate-drafter completion note). Zero canned prong-specific phrasing beyond what the computed state supports.",
...
async function runPipeline(assessment_id: string) {
  try {
    ...
    (report_data as any)._meta = { ...((report_data as any)._meta ?? {}), prompt_version: stampPromptVersion("cppa-risk-assessment", "rebuild-risk") };
