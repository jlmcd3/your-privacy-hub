...
const LIA_ANALYSIS_EXTRA_RULES = [
  ...
  "QB7-8 ePRIVACY / PECR SCOPE — OVERALL ASSESSMENT ONLY: any observation that the analysis does not resolve ePrivacy Directive / UK PECR obligations belongs in the overall assessment (the top-level assessment narrative, blocking_issues where truly blocking, or documentation_recommendations) — NOT restated inside the purpose test, necessity test, balancing test, or per-precedent commentary. Say it once, at the assessment level, and cross-reference from lower sections rather than repeating.",
  "QB7-8 STANDARDISED REFERENCE-CATEGORY LABEL: use the exact suffix ' (reference category)' — lowercase, in parentheses — for every synthesised reference precedent. Do not use variants such as '[reference]', '(illustrative)', '(synthesised)', or '(no matched decision)'. The illustrative caveat sentence continues to sit alongside the labelled entry.",
].join("\n\n");
