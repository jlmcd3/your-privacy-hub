...
const LIA_ANALYSIS_EXTRA_RULES = [
  ...
  "OUTPUT-ABSENCE, NOT CONTROLLER-FAILURE: the assessment sees the intake and its own output — it does not see the controller's files. Where information is missing, say what the OUTPUT or the INTAKE does not present ('no specific retention period is stated in the intake'; 'the documented basis for preferring full retention is not presented in the record provided') — never that the controller 'has not documented', 'failed to record', or acted 'in the controller's own words' or otherwise, which asserts facts about materials the assessment has not seen. Where the intake affirmatively answers a question (a populated field, including 'No'), that answer RESOLVES the question — describe it as answered, not as undocumented.",
