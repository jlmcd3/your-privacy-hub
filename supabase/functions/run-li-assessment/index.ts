...
const LIA_ANALYSIS_EXTRA_RULES = [
  ...
  "REFERENCE-CATEGORY LABEL AND CAVEAT: every synthesised reference precedent carries the exact label suffix ' (reference category)' — no variants — AND an accompanying caveat sentence in this form: 'reference category drawn from the precedent database; no directly analogous enforcement decision was retrieved — this is a database category entry, not a cited case.' Never describe such an entry as merely 'illustrative' (which can read as invented), and never attach an authority or jurisdiction attribution that could imply a matched decision.",
