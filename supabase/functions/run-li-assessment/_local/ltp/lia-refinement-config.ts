...
export const LIA_CRITIC_CONTAINMENT =
  `OUT OF SCOPE — TYPED SURFACES (do not propose findings on these; they are deterministic single-writer surfaces built from the record, and any proposal naming them, at any depth, is refused before it is read): reasonable_expectations, child_factor, public_authority_exclusion, lia_determination, automated_decision_analysis, precedent_class_posture, eprivacy_short_circuit, skeleton_document, rule_applications. Confine every finding to the narrative sections — including the still-model-authored three_part_test, interest_legitimacy, relationship_with_individual, potential_harms and opt_out_feasibility, which remain fully revisable.`;
