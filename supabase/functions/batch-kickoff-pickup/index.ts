...
export const BUILD_STAMP = "pdfexport-1-kickoff-pickup@2026-07-17";
...
Deno.serve(async (req) => {
  ...
  if (decision.kind === "single_flight_skip") {
    ...
    return new Response(JSON.stringify({ decision, build_stamp: BUILD_STAMP }), {
  ...
  // invokeGated does not set x-internal-resume; the orchestrator's internal-resume
  // branch requires that header. Post directly instead:
  ...
  const kickOk = (kickRes as Response).ok === true;
  const kickStatus = (kickRes as Response).status ?? 0;
  await logRun({
    decision: "kick",
  ...
  return new Response(JSON.stringify({ decision, kick_ok: kickOk, kick_status: kickStatus, build_stamp: BUILD_STAMP }), {
    headers: { ...cors, "Content-Type": "application/json" },
  });
});
