...
export const DPIA_TOOL_MODULE: ToolModule = {
  ...
  extraRules: [
    ...
    "RESIDUAL-RISK CONSISTENCY RULE: if a section instructs the organisation to re-score residual risk before sign-off, any residual-risk levels stated elsewhere must be labelled 'proposed — subject to the organisation's re-scoring', not presented as final.",
    ...
    "OUTSIDE-THE-EEA TRANSFER PHRASING: when flagging Chapter V transfer obligations, write \"outside the EEA and the UK (i.e. a third country without an adequacy decision)\" rather than \"outside the EEA\" alone. The UK is outside the EEA post-Brexit, but EEA↔UK flows are adequacy-covered in both directions, so an unqualified \"non-EEA\" wrongly implies Chapter V mechanisms are needed for UK processing. Name the mechanisms as EU SCCs, the UK IDTA, or an Article 45 adequacy decision (including the EU–US Data Privacy Framework where the importer is certified).",
    ...
    "DEVICE-ACCESS SCOPE PRECISION: ePrivacy Article 5(3) / PECR Regulation 6 apply to STORING or ACCESSING information ON a terminal device (cookies, local storage, SDKs, fingerprinting scripts). IP addresses transmitted from a device and server-side event logs are not, by themselves, Article 5(3) device access. When flagging the ePrivacy gate, state that it applies IF collection involves storing/accessing information on the device, and direct the user to confirm whether any client-side instrumentation triggers it — do not assert that all IP-address or event-log processing requires Article 5(3) consent.",
  ].join("\n\n"),
