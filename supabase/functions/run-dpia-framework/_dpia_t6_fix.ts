// DPIA-T6-FIX-TURN (2026-07-25) — deterministic post-pass discharging the
// T6-NONCPPA-MEASUREMENT-BATCH-1 DPIA per-tool backlog items 81(a)+(b).
//
// Mirrors run-li-assessment/_lia_t6_fix.ts (ledger item 89) exactly,
// adapted to the DPIA generator. Runs AFTER _w1_dpia_wire and BEFORE the
// LEAK-PREV-P1 emit gate + P2 serializer.
//
...
//   `proposition_key` resolves in DPIA_VERIFIED_AUTHORITIES and applies
//   write-around (null pinpoints + write_around=true) for objects on
//   DPIA_UNANCHORED_PROPOSITIONS. It does NOT touch objects whose key is
...
export interface DpiaT6FixCounters {
  version: string;
  stamp: string;
  build_stamp?: string;
...
const STOPWORDS = new Set<string>([
  ...
  "business","subject","data","processing","assessment","impact","dpia",
...
function auditNode(node: Record<string, unknown>, c: DpiaT6FixCounters): void {
  ...
  if (pk && DPIA_VERIFIED_AUTHORITIES[pk] && isVerified) {
