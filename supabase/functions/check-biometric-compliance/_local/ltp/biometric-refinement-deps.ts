// ITEM 412 LEG D — shared refinement dependencies for the Biometric product.
//
// Mirrors cyber-refinement-deps.ts / governance-refinement-deps.ts /
// admt-refinement-deps.ts / lia-refinement-deps.ts exactly. The generation
// model is passed in EXPLICITLY by the caller (`currentGenerationModel()`
// resolved at the call site, inside the request context) rather than read
// lazily inside the critic closure — the resurrection-bug class: a closure that
// resolves the model later can fall out of the request's AsyncLocalStorage
// context and silently default to sonnet.
//
// Metering is identical to the other six products: one `api_usage` row per
...
export const BIOMETRIC_REFINEMENT_ENABLED = true;

export function makeBiometricRefinementDeps(
  sourceRowId: string | null,
  /** REQUIRED — the run's resolved generation model. Never defaulted here. */
  generationModel: string,
  fnLabel = "check-biometric-compliance",
): RefinementDeps {
