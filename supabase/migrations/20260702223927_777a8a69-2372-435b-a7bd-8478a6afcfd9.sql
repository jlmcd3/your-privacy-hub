...
begin
  ...
  loop
    -- Reset any "running" jobs stuck >12 minutes back to pending.
    -- Threshold MUST exceed run-stress-job's 9-minute tool timeout so we
    -- never preempt a legitimately in-flight generator (CPPA Risk in
    -- particular routinely runs 3-6 minutes).
    with reset as (
      update public.static_stress_jobs
         set status = 'pending', started_at = null, error_message = 'reset by watchdog (stuck >12m)'
