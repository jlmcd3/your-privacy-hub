CREATE OR REPLACE FUNCTION public.admin_fire_ingest_edpb_oss(
  ...
  p_fetch_pdf_text boolean DEFAULT false
) RETURNS bigint
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT net.http_post(
    ...
    body := jsonb_build_object(
      ...
      'fetch_pdf_text', p_fetch_pdf_text
    ),
...
REVOKE EXECUTE ON FUNCTION public.admin_fire_ingest_edpb_oss(text, integer, integer, boolean) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_fire_ingest_edpb_oss(text, integer, integer, boolean) TO service_role;