...
-- Update the existing user-update RLS policy to also lock these new fields
...
WITH CHECK (
  ...
  AND NOT (subscription_plan IS DISTINCT FROM (SELECT p.subscription_plan FROM profiles p WHERE p.id = auth.uid()))
  AND NOT (subscription_interval IS DISTINCT FROM (SELECT p.subscription_interval FROM profiles p WHERE p.id = auth.uid()))
);