...
- **Reserved-framing law (UNCHANGED AS THE DEFAULT):** counsel-reserved determinations are stated as reserved to the Company and its counsel. The tool states the standard and the record; it never green-lights. This remains the governing rule for **every** determination in this codebase and across the fleet — ADMT, DPIA, LIA, governance, IR-playbook, biometric, cyber, registration, and every other reserved-framing section — and nothing below relaxes it for any of them.
  - **ONE NAMED EXCEPTION — § 7156(a) comparable-set bundling call, cppa-risk only (CEO-authorized, 2026-08-01, Item 319 + addendum).** For the narrow question of whether a **secondary** activity may be bundled into this assessment or should get its own, the tool states a **recommendation**: any one of the five § 7156(a)(1)-derived comparison dimensions recorded as different ⇒ "Recommended: conduct a separate risk assessment"; none different but one or more unresolved ⇒ recommend a separate assessment unless the open dimensions are confirmed the same; none different and none unresolved ⇒ "can be addressed within this single assessment." Binding conditions: the underlying dimension-by-dimension comparison is always shown alongside the recommendation (the recommendation is **additive**, never a replacement for showing the work); the language is **"we recommend"**, never "the law requires" — an operational rule may not be dressed up as statutory text (the exact defect the Item-276 RIDER already corrected once); and the closing not-legal-advice line is mandatory. The threshold is pinned by `item319-secondary-recommendation.test.ts` and the copy parity by `item319-addendum-copy-parity.test.ts`.
  - **Scope of the exception is exhaustive.** It reaches the bundling/comparable-set call for secondary activities and nothing else. It does **not** reach the primary activity's five § 7152 analytic deliverables, any other § 7156 question, any other cppa-risk determination, or any other product. Extending it requires a separate CEO authorization recorded here; absent that, the default rule above controls.
