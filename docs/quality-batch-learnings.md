...
## 2026-07-22 — batch d7cd2ff0

10. GOVERNANCE — ambiguous multi-product tool entries (run b0024821): a fixture tools[] entry 'Otter.ai / Fireflies' produced vendor-count hallucination flags. Intake-guidance implication: the tools field on the governance intake page should instruct one product per entry; users listing alternatives ('X / Y') create ambiguity the analysis inherits. Deterministic fixture rule shipped in parallel.
11. REGISTRATION — fee-tier self-service (run 7edf09b4): the intake already collects staff count and revenue band, which resolve the ICO fee tier deterministically; the report should never ask the user to confirm what the intake resolves. Engine fix shipped. Intake-guidance implication: none — validates that richer intake fields let the product answer more.
12. IR PLAYBOOK — date-regime coherence (run 09d3c549, score 81): a year-old discovery date produced statute-vintage confusion. Deterministic 30-day recency validation shipped for fixtures. Intake-guidance implication (2nd sighting, upgrade from hypothesis): the customer intake page should warn when discoveryDateTime is >30 days old — notification clocks have likely expired and the playbook's framing changes.

## 2026-07-23 — Turn A1 (QB-P25 Item 3) — DPA drafting record

13. DPA — grader-invisible drafting record shipped. `generate-dpa` now emits a third `===DRAFTING_RECORD===` block after `===ANNOTATIONS===`; `parseDpa` splits it and persists it on `report_data._drafting_record`. The block is suppressed at TWO enumerated choke points, not by prefix rule:
    - **Grader payload strip** — `METADATA_KEYS` in `supabase/functions/_shared/grader/payload.ts` (enumerated list; `_drafting_record` added). `buildGraderPayload` iterates and skips.
    - **Prose-lint / blacklist scan skip** — `_RESERVED_KEYS` in `supabase/functions/_shared/advisory-voice.ts` (enumerated `Set`; `_drafting_record` added). `extractProseFromReport` walks keys and returns early on match.
    Rubric note: golden set `_shared/golden/dpa.ts` now asserts on every DPA case that `===DRAFTING_RECORD===`, `framework_selection`, and `clause_deviations` never appear in `document_text` — leakage of the private block into the persisted body is a regression, not a soft finding. Contract test at `generate-dpa/drafting-record.test.ts` exercises both strip points and the three-block split.

## Learning 14 (2026-07-23, QB-P25 Turn A2 — Cyber schema)

CPPA Cybersecurity controls now emit three additional customer-facing fields: `evidence` (named intake artefact supporting the score), `differentiator` (why this control's score differs from siblings, or why uniformity reflects evidentiary depth), and `rank` (unique 1..18 integer, 1 = highest reader-priority). These are DESIGNED OUTPUT, never internal reasoning traces. `next_steps` is now an object array `{ text, owner, trigger }` capped at 3. The uniform-scores rationale rule was replaced by per-control differentiators. Server-side normalisation renumbers `rank` deterministically by (status severity, score) so half-batch merges produce a clean 1..N run. Renderer sorts the control accordion by `rank`.

## Learning 18 (2026-07-23, QB-P25 Turn A3 — ADMT determination_basis + exposure enum)

ADMT reconciles the previously-conflicting rules at index.ts:188/238 (definitive true/false) and :284 (conservative in-scope when ambiguous) via a new REQUIRED field `scope_analysis.determination_basis ∈ {"established","conservative_assumption"}`. Boolean `triggers_significant_decision` stays definitive; TRUE-from-ambiguity MUST set `conservative_assumption`, TRUE from an affirmatively-enumerated § 7001(ddd) category (or FALSE from an established out-of-scope facts pattern such as solely_advertising) MUST set `established`. Under `conservative_assumption`, `notice_gaps`/`opt_out_gaps`/`access_gaps` render in COMPACT mode — each entry contains ONLY `{element_id, element, duty_if_in_scope, citation}`; status/finding/remediation/enforcement_exposure/sample_language are omitted and Pass-2 sample-language drafting is skipped entirely. Element_id vocabulary and array names are UNCHANGED (grader + open-items contracts preserved). Under `established`, the FULL entry shape is emitted and `enforcement_exposure` is enum-only: `"per_violation" | "per_consumer_scalable" | "na"` — free-form dollar figures live EXCLUSIVELY in `enforcement_context`. A post-generation normalizer coerces legacy exposure strings to enum and strips non-compact keys under conservative mode; missing/invalid `determination_basis` defaults to `established` for backward compatibility. Golden fixtures now cover both branches (established credit + hiring, established out-of-scope advertising, conservative service-eligibility).
