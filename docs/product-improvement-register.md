...
### 18. [PROCESS] Golden-fixture CI enforcement — no pre-commit hook (2026-07-23)
Per FIXTURE-LIA-3 meta-question: `_tests/golden-contract.test.ts` correctly catches nested required-always violations (verified by making it fail on the Northwind Retail intake pre-fix, then pass post-fix). The `CONTRACT_BY_TOOL` hand-mirror in the test file is complete; the `validateIntake` walker correctly evaluates nested paths. The Wave-4 leak happened because no automated CI gate ties this test to golden-fixture commits — the R-TURN-3 turn that added the Northwind absence-convention adversarial did not run the golden-contract test before landing. Durable fix: wire a pre-deploy / pre-commit hook (or lovable-exec test gate) that runs `_tests/golden-contract.test.ts` and `_tests/qbp20.test.ts` whenever any file under `supabase/functions/_shared/golden/` or `supabase/functions/_shared/intake-contracts/` changes.
