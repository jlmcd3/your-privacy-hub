...
**Last updated:** 2026-07-24T17:51:06Z — `REGISTRY-VERBATIM-AUDIT` executed (dropped-order restore). ADMT `_shared/registry/admt-verified-authorities.ts` audited against `cppa_authorities.full_text` (normalized: smart→straight quotes, dash fold, whitespace collapse): **0/34 EXACT**, 3 partial-window (head/tail-80 char) matches, 31 NOT_FOUND — all 34 rows are paraphrases labelled `verbatim_quote` (same defect class as the TURN C fabricated `§ 7222(c)` row). Biometric `_shared/registry/biometric-statute-registry.ts`: 46/46 pass pinpoint-in-quote self-consistency + all `primary_source_url` well-formed; corpus-pin **infeasible** (no BIPA/CUBI/RCW/GDPR text mirror) — deferred pending biometric corpus ingestion. Shipped: (a) `docs/courier/REGISTRY-VERBATIM-AUDIT-2026-07-24.md` report; (b) `src/registry/__tests__/admt-verified-authorities-corpus-pin.test.ts` allow-list CI (frozen `KNOWN_PARAPHRASED_KEYS` of all 34 keys; skips when `PGHOST` unset; fails on any new failing row OR any listed row starting to pass — forces set to shrink to empty). Standing rule adopted: **no registry row lands without corpus verification** (extends to any future risk/cyber/DPIA/IR verified-authority registry). §4 below rewritten to LIA revert HEAD state (`d7efef9`). Anti-drop accounting: all three dispatch orders (audit / §4 correction / cyber shape-test fold-in for next SAMPLES turn) accounted for.
