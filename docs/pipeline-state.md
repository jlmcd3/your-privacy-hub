...
14. After samples 8/8 — `REGISTRATION-INTAKE-CONTRACT-RAIL-MAP` (pre-approved authoring turn per overnight standing order: shared contract + rail-map + corpus-cited rails + dummy data).
15. Deferred — orchestrator → `delivery_contracts` wiring (queued between waves; priority raised — second isolate death in one day, see §5-adjacent).
16. Deferred — WAVE12-FIX TURN C / C3 (fallback-density expansion) — needs wave-12 propositional list; queue for morning CEO or next wave's finding digest.
