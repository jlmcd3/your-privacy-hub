...
**Last updated:** 2026-07-24T18:15:19Z — `SAMPLES-CONTRACT-ir_playbook` (7/8) DONE (frontend/test only, no deploy). Reconciled `F_IR_EU` (audited surface `invoke_body_extras`) and `F_IR_US` (audited surface `invoke_body`) in `src/lib/sampleFixtures.ts` against `irPlaybookContract`. Added missing required-always `organizationName` to both variants ("Silver & Gold Cold Storage A/S", "Busted Sled Solutions, Inc."). Normalized drifted enum/multi-enum values to contract literals: EU `cause` narrative → "Ransomware or malware" (IR_CAUSES); EU `dataTypes` free-text array → ["Names and contact details"] (IR_DATA_TYPES); EU `affectedCount` "approximately 1,900…" → "1,000–10,000" (IR_COUNTS); EU `contained` narrative → "Yes" (IR_CONTAINED); EU `organisationType` "Cold-chain logistics operator (Denmark)" → "Company" (IR_ORG_TYPES); US `cause` narrative → "Phishing / credential compromise" (IR_CAUSES; credential-stuffing best-fit); US `dataTypes` free-text array → ["Names and contact details","Biometric data"] (IR_DATA_TYPES); US `affectedCount` "approximately 8,400…" → "1,000–10,000" (IR_COUNTS); US `contained` narrative → "Yes" (IR_CONTAINED); US `organisationType` "Logistics-technology company (US multi-state)" → "Company" (IR_ORG_TYPES). Preserved every dropped/normalized string verbatim in each fixture's non-audited `scenario_summary` (EU: "driver schedules", "refrigerated-shipment manifests", "~1,900 employees and contractors", "cold-chain logistics operator (Denmark)", full ransomware/containment narrative; US: "shipment-tracking account data", "linked fingerprint time-clock records", "~8,400 consumer accounts", "logistics-technology company (US multi-state)", credential-stuffing/containment narrative). No unknown-key STOP triggered (audited surfaces contained only contract keys after normalization; live-form audit of `src/pages/IRPlaybook.tsx` L91-97 + L144 invoke confirms the form emits only the 10 contract keys). Flipped `ir_playbook` out of `SAMPLE_ADVISORY_TOOLS` in `supabase/functions/_tests/contract-surface-audit.test.ts` → FATAL tier; counter updated to 7/8; remaining ADVISORY set: `dpa`, `biometric`. **Tests green:** deno `contract-surface-audit.test.ts` 3/3 ok (ir_playbook absent from ADVISORY drift and FATAL failures; ADVISORY now `biometric` only — 2 pre-existing biometric rows unchanged); vitest `sampleFixtures.shape.test.ts` 50 passed / 2 failed (both failures = pre-existing `cppa_cyber/us-supplemental` missing `company_name/profile_industry/profile_audit/industry_sector`, OUT OF SCOPE per dispatch). No edge deploy this turn.
...
11. **NEXT** — `SAMPLES-CONTRACT-biometric` (8/8). Frontend/test only; not deploy-locked. Note: 2 biometric ADVISORY rows currently drift (unknown key `is_free_tier`; multi-enum `biometricTypes` element `"fingerprint"`; free-text `orgType` and long `purpose` narrative) — full reconciliation queued for this turn.
12. Then — `SAMPLES-CONTRACT-dpa` closes the series.
