...
**Last updated:** 2026-07-24T17:34:54Z — WAVE12-FIX TURN E (cppa-cyber) DEPLOYED with fresh-clock BUILD_STAMP `w12-cyber-turne@2026-07-24T17:32:35Z`. E1a root-cause fix in `_w6_cyber_fix.ts` (sentence splitter no longer breaks on `;`; orphan-preposition rewrites roll back to omission rather than emit "…on;" stubs). E1b defensive post-assembly guard in new `_w12_cyber_e1.ts` (`sanitizeCrosswalkText` / `applyW12CyberE1`): strips mid-string orphan-prep-then-terminator stubs (exact wave-12 fragment "the ISO 27001 framework provides comparative guidance on;" scrubbed), drops any sentence with unbalanced parens, deduplicates operative sentences whitespace/case-normalised. Fail-open. Telemetry (`crosswalk_fragments_dropped` / `unbalanced_parens_dropped` / `crosswalk_dupes_removed` / `surfaces_scanned` / `sentences_scanned`) lands ONLY under `_meta.internal.crosswalk` — never on customer surfaces (TURN C `_w<digits>_*` strip continues to guard). Retro-audit of TURN A: `stripLiteralNPlaceholder` and `attachCyberAggregates` intact and untouched. 45/45 tests green (`_w12_turne.test.ts` 16/16 + `_w6_cyber_fix.test.ts` 25/25 + `_w10_cyber_aggregates.test.ts` 5/5 — includes fail-open pass-through, mid-string stub scrub, unbalanced-paren drop, dedupe, telemetry-placement, and no-customer-surface-leak assertions). Deploy 17:34Z (edge deploy tool confirmed).
