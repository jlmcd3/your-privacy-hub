...
import { runPass1Llm, PASS1_MANIFEST } from "../../../../supabase/functions/run-cppa-risk-assessment/_local/ltp/pass1-llm.ts";
import { renderTemplate, assertCalibrationMatch } from "../../../../supabase/functions/run-cppa-risk-assessment/_local/ltp/pass2-render.ts";
import { resolveSlot } from "../../../../supabase/functions/run-cppa-risk-assessment/_local/ltp/slot-resolver.ts";
import { derivePlan } from "../../../../supabase/functions/run-cppa-risk-assessment/_local/ltp/derive.ts";
import { PASS2_TEMPLATES, FIRM_VARIANT_CLOSENESS_MAX } from "../../../../supabase/functions/run-cppa-risk-assessment/_local/ltp/content/pass2-templates.ts";
...
Deno.test("all 47 templates enumerated (DOC 157 adds 2: § 7150(b)(3) not-significant + housing-excluded postures; ITEM 241.3 adds 7: golden four-move action, record_sufficiency prose lead-in, 5 CP5 §3.2 section openers; ITEM 276 adds 2: primary-subject lead + § 7156(a) segmentation; ITEM 244 adds 5: LIA present/silent, processing_narrative, record_sufficiency prose v2, scope opener v2; PN-CORPUS-L-RISK-1 adds 1: § 7150(b)(2)(A) exempt posture; DOC 154 adds 2: § 7150(b)(2) unresolved + § 7150(b)(3) evaluation postures)", () => {
  assertEquals(Object.keys(PASS2_TEMPLATES).length, 47);
});
