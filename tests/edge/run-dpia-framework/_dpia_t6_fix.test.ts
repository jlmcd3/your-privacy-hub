// DPIA-T6-FIX-TURN — colocated Deno tests. Mirrors LIA-T6-FIX-TURN pin list
// (ledger item 89). Regression pins derived from T6 batch f2ac3a26 dpia
// failure classes (citation_misapplied ×2 HIGH, unsupported_business_claim
// ×1 HIGH, internal_reasoning_leak ×1 HIGH).

import {
  assert, assertEquals, assertStringIncludes,
} from "https://deno.land/std@0.224.0/assert/mod.ts";
import { applyDpiaT6Fix, _internals, DPIA_T6_FIX_STAMP } from "../../../supabase/functions/run-dpia-framework/_dpia_t6_fix.ts";

// ── Class A: truncated citation string is nulled (no proposition_key) ──
Deno.test("Class A: truncated 'Art. 35(' citation is nulled", () => {
  const report: any = {
    section_2_necessity: {
      analysis: {
        annotations: [{ text: "..." }],
        citation: "GDPR Art. 35(",
        subsection: "GDPR Art. 35(",
        verbatim_quote: null,
      },
    },
  };
  const c = applyDpiaT6Fix(report);
  assertEquals(report.section_2_necessity.analysis.citation, null);
  assertEquals(report.section_2_necessity.analysis.pinpoint_omitted, true);
...
Deno.test("Class A: unresolvable key nulls invented pinpoint", () => {
  ...
  const c = applyDpiaT6Fix(report);
  assertEquals(report.risk_assessment.citation, null);
  assertEquals(report.risk_assessment.subsection, null);
  assertEquals(report.risk_assessment.verbatim_quote, null);
  assertEquals(report.risk_assessment.citation_verified, false);
  assert(c.classA_pinpoint_omissions >= 1);
});
...
Deno.test("Class B: unsupported claim downgraded, prior sentence intact", () => {
  ...
  const c = applyDpiaT6Fix(report, { intake });
...
Deno.test("Anchor keys (citation/verbatim_quote) never treated as prose", () => {
  const report: any = {
    node: {
      ...
      citation: "GDPR Art. 35",
      subsection: "GDPR Art. 35(1)",
      verbatim_quote: "the system demonstrates that processing is necessary for the DPIA",
    },
  };
  applyDpiaT6Fix(report, { intake: {} });
  assertStringIncludes(
...
Deno.test("Idempotent: second pass makes no additional content changes", () => {
  ...
  const c2 = applyDpiaT6Fix(report, { intake: {} });
...
Deno.test("Fail-open on malformed input", () => {
  const c1 = applyDpiaT6Fix(null as any);
  assertEquals(c1.errors, 0);
  const c2 = applyDpiaT6Fix({ circ: {} } as any);
  assertEquals(typeof c2.strings_scanned, "number");
});

// ── _meta.internal.dpia_t6fix telemetry written ──
Deno.test("Telemetry: _meta.internal.dpia_t6fix written", () => {
  const report: any = { body: { text: "hello" } };
  applyDpiaT6Fix(report, { intake: {}, buildStamp: "test-stamp" });
  assertEquals(typeof report._meta.internal.dpia_t6fix, "object");
  assertEquals(report._meta.internal.dpia_t6fix.stamp, DPIA_T6_FIX_STAMP);
  assertEquals(report._meta.internal.dpia_t6fix.build_stamp, "test-stamp");
});

// ── _meta preserved (not clobbered) ──
Deno.test("_meta.internal preexisting keys preserved", () => {
  const report: any = { _meta: { internal: { dpia_w1_wire: { existing: true } } } };
  applyDpiaT6Fix(report);
  assertEquals(report._meta.internal.dpia_w1_wire.existing, true);
  assert(report._meta.internal.dpia_t6fix);
});

// ── Truncated-citation detector unit ──
Deno.test("isTruncatedCitation: shapes", () => {
  const f = _internals.isTruncatedCitation;
  assert(f("GDPR Art. 35("));
  assert(f("Article"));
  assert(f("GDPR Art."));
  assert(f("Regulation (EU) 2016/679, Art. 35(1)(f"));
  assert(f("§ 4,"));
  assert(!f("GDPR Art. 35(1)"));
