...
W1 Invented entities: processors, vendors, technologies, certifications, or workflows not in the intake — EXCEPT persuasive authority citations, which are sourced from the authority_hooks corpus and are never hallucinations (doc 230 §6.2; applies only when DPIA_HOOKS_ENABLED is true and DPIA_HOOKS carries a ratified row — dark by default) (history: invented monitoring vendors and HR-review workflows).
...
Deno.test("item377 (d2) — designed-output exemplars byte-verbatim in verifier prompt", () => {
  const BLOCK = `DESIGNED-OUTPUT PATTERNS (these are deliberate product output; a proposal altering any of them fails condition 4 and must be REJECTED): the final disclaimer; quoted statutory text; "[TO BE COMPLETED …]"/"[TO BE ASSESSED]" placeholders where the intake is silent on the item; "(default — confirm)" markers; the canonical closes "…further clarification is advisable." / "…further internal investigation is advisable."; drafting-voice references to "the record"; the EDPB DPIA template v1.0 structure and its § 0.5 assessment-team/validation fields; plain-prose FSOR/Agency-position citations; corpus-verified recent law (SB 446 notice windows; Cal. Civ. Code § 1798.140(ag); UK GDPR Art. 6(11) per the DUAA 2025); persuasive authority citations in the form "In [Authority], [Regulator] found/states/advised that where [fact pattern], [finding]. … ([citation]; [status].)" appended to the executive summary or the necessity-and-proportionality analysis — these are hook citations from the authority_hooks corpus (doc 230/232, dark behind DPIA_HOOKS_ENABLED) and must not be flagged as hallucinations, invented entities, or citation defects; an "information_needed" entry whose text is the ratified two-leg-disagreement template ("The specificity of your answer here is important to check whether a relevant regulator action…") is expected behaviour, not a defect, and must not be deducted for. Conversely: a placeholder or absence statement covering something the intake DOES supply is NOT protected — that is a record-contradiction, and its correction should be APPROVED when conditions 1–3 hold.`;
